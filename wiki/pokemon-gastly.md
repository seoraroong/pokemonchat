---
title: "Gastly / 고오스"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-ghost", "type-poison", "evo-line-gastly"]
aliases: ["고오스", "gastly"]
summary: "#092 고오스 (고스트 / 독) — GO 최대 CP: 1,229"
evolution_line: "gastly"
---

# Gastly / 고오스

## Definition / 정의
**고오스** (#092)는 일반 포켓몬로, [[type-ghost|고스트]], [[type-poison|독]] 타입입니다.
"가스로 만들어진 몸은 어떠한 크기의 상대라도 둘러싸서 숨을 못 쉬게 한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #092 |
| 영문 이름 | Gastly |
| 타입 | [[type-ghost|고스트]], [[type-poison|독]] |
| 키 | 1.3m |
| 몸무게 | 0.1kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 190 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 30 |
| 공격 | 35 |
| 방어 | 30 |
| 특수공격 | 100 |
| 특수방어 | 35 |
| 스피드 | 80 |
| **합계** | **310** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-haunter|고우스트]] |
| 계열 루트 | [[pokemon-gastly]] |

## 특성 (Abilities)
- levitate

## Related Concepts / 관련 개념
- [[type-ghost|고스트 타입]] — 고오스의 타입
- [[type-poison|독 타입]] — 고오스의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 186 |
| 방어 (GO) | 67 |
| 체력 (GO) | 102 |
| 최대 CP (Lv.40) | 1,229 |
| 최대 CP (Lv.50) | 1,390 |
| 타입 | 고스트 / 독 |
| 버디 거리 | 3km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅, 고스트, 에스퍼, 악 |
| 저항 | 독(×¼), 벌레(×¼), 풀, 페어리 |
| 무효 | 노말, 격투 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-astonish|놀래키기]], [[move-lick|핥기]], [[move-sucker-punch|기습]]
- **스페셜 기술:** [[move-dark-pulse|악의파동]], [[move-night-shade|나이트헤드]], [[move-ominous-wind|괴상한바람]], [[move-sludge-bomb|오물폭탄]]  *(커뮤니티 데이/전용: [[move-sucker-punch|기습]], [[move-ominous-wind|괴상한바람]])*

### 진화 체인
- **진화 후:** [[pokemon-haunter|고우스트]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| GASTLY_SHADOW | 186 | 67 | 102 | 1,229 |
## References
- Source: PokéAPI v2 pokemon/92
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
