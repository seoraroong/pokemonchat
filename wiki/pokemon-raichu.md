---
title: "Raichu / 라이츄"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-electric", "evo-line-pikachu"]
aliases: ["라이츄", "raichu"]
summary: "#026 라이츄 (전기) — GO 최대 CP: 2,182"
evolution_line: "pikachu"
---

# Raichu / 라이츄

## Definition / 정의
**라이츄** (#026)는 일반 포켓몬로, [[type-electric|전기]] 타입입니다.
"전기가 모이면 근육이 자극되어 여느 때보다 공격적이 된다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #026 |
| 영문 이름 | Raichu |
| 타입 | [[type-electric|전기]] |
| 키 | 0.8m |
| 몸무게 | 30.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 75 |


## 도감 설명

> 전기가 모이면 근육이 자극되어 여느 때보다 공격적이 된다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 60 |
| 공격 | 90 |
| 방어 | 55 |
| 특수공격 | 90 |
| 특수방어 | 80 |
| 스피드 | 110 |
| **합계** | **485** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-pikachu|피카츄]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-pikachu]] |

## 특성 (Abilities)
- static
- lightning-rod

## Related Concepts / 관련 개념
- [[type-electric|전기 타입]] — 라이츄의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 193 |
| 방어 (GO) | 151 |
| 체력 (GO) | 155 |
| 최대 CP (Lv.40) | 2,182 |
| 최대 CP (Lv.50) | 2,467 |
| 타입 | 전기 |
| 버디 거리 | 1km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|202위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|219위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅 |
| 저항 | 비행, 강철, 전기 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-spark|스파크]], [[move-thunder-shock|전기쇼크]], [[move-volt-switch|볼트체인지]], [[move-charm|애교부리기]]
- **스페셜 기술:** [[move-brick-break|깨트리다]], [[move-thunder|번개]], [[move-thunder-punch|번개펀치]], [[move-wild-charge|와일드볼트]], [[move-skull-bash|로케트박치기]], [[move-trailblaze|Trailblaze]], [[move-surf|파도타기]]  *(커뮤니티 데이/전용: [[move-thunder|번개]])*

### 진화 체인
- **진화 전:** [[pokemon-pikachu|피카츄]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| RAICHU_ALOLAN | 201 | 154 | 155 | 2,286 |
## References
- Source: PokéAPI v2 pokemon/26
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
