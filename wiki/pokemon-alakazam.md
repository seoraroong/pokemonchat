---
title: "Alakazam / 후딘"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-psychic", "evo-line-abra"]
aliases: ["후딘", "alakazam"]
summary: "#065 후딘 (에스퍼 타입) — 기본 스탯 합계 500"
evolution_line: "abra"
---

# Alakazam / 후딘

## Definition / 정의
**후딘** (#065)는 일반 포켓몬로, [[type-psychic|에스퍼]] 타입입니다.
"뇌세포는 항상 분열해서 죽을 때까지 늘어나기 때문에 모든 것을 기억해 놓을 수 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #065 |
| 영문 이름 | Alakazam |
| 타입 | [[type-psychic|에스퍼]] |
| 키 | 1.5m |
| 몸무게 | 48.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 50 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 55 |
| 공격 | 50 |
| 방어 | 45 |
| 특수공격 | 135 |
| 특수방어 | 95 |
| 스피드 | 120 |
| **합계** | **500** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-kadabra|윤겔라]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-abra]] |

## 특성 (Abilities)
- synchronize
- inner-focus
- magic-guard

## Related Concepts / 관련 개념
- [[type-psychic|에스퍼 타입]] — 후딘의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 271 |
| 방어 (GO) | 167 |
| 체력 (GO) | 146 |
| 최대 CP (Lv.40) | 3,057 |
| 최대 CP (Lv.50) | 3,456 |
| 타입 | 에스퍼 |
| 버디 거리 | 3km |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|296위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-confusion|염동력]], [[move-psycho-cut|사이코커터]], [[move-counter|카운터]]
- **스페셜 기술:** [[move-dazzling-gleam|매지컬샤인]], [[move-fire-punch|불꽃펀치]], [[move-focus-blast|기합구슬]], [[move-future-sight|미래예지]], [[move-psychic|사이코키네시스]], [[move-shadow-ball|섀도볼]]  *(커뮤니티 데이/전용: [[move-counter|카운터]], [[move-dazzling-gleam|매지컬샤인]], [[move-psychic|사이코키네시스]])*

### 진화 체인
- **진화 전:** [[pokemon-kadabra|윤겔라]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| ALAKAZAM_MEGA | 367 | 207 | 146 | 4,510 |
| ALAKAZAM_SHADOW | 271 | 167 | 146 | 3,057 |
## References
- Source: PokéAPI v2 pokemon/65
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
