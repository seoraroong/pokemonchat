---
title: "Kabutops / 투구푸스"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-rock", "type-water", "evo-line-kabuto"]
aliases: ["투구푸스", "kabutops"]
summary: "#141 투구푸스 (바위 / 물) — GO 최대 CP: 2,713"
evolution_line: "kabuto"
---

# Kabutops / 투구푸스

## Definition / 정의
**투구푸스** (#141)는 일반 포켓몬로, [[type-rock|바위]], [[type-water|물]] 타입입니다.
"수중을 이동할 때 손발을 작게 접고 등껍질을 구부려 빠르게 헤엄친다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #141 |
| 영문 이름 | Kabutops |
| 타입 | [[type-rock|바위]], [[type-water|물]] |
| 키 | 1.3m |
| 몸무게 | 40.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 60 |
| 공격 | 115 |
| 방어 | 105 |
| 특수공격 | 65 |
| 특수방어 | 70 |
| 스피드 | 80 |
| **합계** | **495** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-kabuto|투구]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-kabuto]] |

## 특성 (Abilities)
- swift-swim
- battle-armor
- weak-armor

## Related Concepts / 관련 개념
- [[type-rock|바위 타입]] — 투구푸스의 타입
- [[type-water|물 타입]] — 투구푸스의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 220 |
| 방어 (GO) | 186 |
| 체력 (GO) | 155 |
| 최대 CP (Lv.40) | 2,713 |
| 최대 CP (Lv.50) | 3,067 |
| 타입 | 바위 / 물 |
| 버디 거리 | 5km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|495위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|365위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 격투, 땅, 풀(×4), 전기 |
| 저항 | 노말, 비행, 독, 불꽃(×¼), 얼음 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-fury-cutter|연속자르기]], [[move-mud-shot|머드숏]], [[move-rock-smash|바위깨기]], [[move-waterfall|폭포오르기]]
- **스페셜 기술:** [[move-ancient-power|원시의힘]], [[move-stone-edge|스톤에지]], [[move-water-pulse|물의파동]], [[move-aqua-jet|아쿠아제트]]  *(커뮤니티 데이/전용: [[move-fury-cutter|연속자르기]])*

### 진화 체인
- **진화 전:** [[pokemon-kabuto|투구]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| KABUTOPS_SHADOW | 220 | 186 | 155 | 2,713 |
## References
- Source: PokéAPI v2 pokemon/141
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
