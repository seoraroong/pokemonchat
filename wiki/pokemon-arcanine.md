---
title: "Arcanine / 윈디"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-fire", "evo-line-growlithe"]
aliases: ["윈디", "arcanine"]
summary: "#059 윈디 (불꽃 타입) — 기본 스탯 합계 555"
evolution_line: "growlithe"
---

# Arcanine / 윈디

## Definition / 정의
**윈디** (#059)는 일반 포켓몬로, [[type-fire|불꽃]] 타입입니다.
"옛날부터 많은 사람의 마음을 사로잡은 아름다운 포켓몬이다. 날듯이 경쾌하게 달린다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #059 |
| 영문 이름 | Arcanine |
| 타입 | [[type-fire|불꽃]] |
| 키 | 1.9m |
| 몸무게 | 155.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 75 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 90 |
| 공격 | 110 |
| 방어 | 80 |
| 특수공격 | 100 |
| 특수방어 | 80 |
| 스피드 | 95 |
| **합계** | **555** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-growlithe|가디]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-growlithe]] |

## 특성 (Abilities)
- intimidate
- flash-fire
- justified

## Related Concepts / 관련 개념
- [[type-fire|불꽃 타입]] — 윈디의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 227 |
| 방어 (GO) | 166 |
| 체력 (GO) | 207 |
| 최대 CP (Lv.40) | 3,029 |
| 최대 CP (Lv.50) | 3,425 |
| 타입 | 불꽃 |
| 버디 거리 | 3km |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|207위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-bite|물기]], [[move-fire-fang|불꽃엄니]], [[move-snarl|바크아웃]], [[move-thunder-fang|번개엄니]]
- **스페셜 기술:** [[move-bulldoze|땅고르기]], [[move-crunch|깨물어부수기]], [[move-fire-blast|불대문자]], [[move-flamethrower|화염방사]], [[move-wild-charge|와일드볼트]], [[move-psychic-fangs|사이코팽]], [[move-scorching-sands|열사의대지]]  *(커뮤니티 데이/전용: [[move-bite|물기]], [[move-bulldoze|땅고르기]])*

### 진화 체인
- **진화 전:** [[pokemon-growlithe|가디]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| ARCANINE_HISUIAN | 232 | 165 | 216 | 3,145 |
| ARCANINE_SHADOW | 227 | 166 | 207 | 3,029 |
## References
- Source: PokéAPI v2 pokemon/59
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
