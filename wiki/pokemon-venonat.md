---
title: "Venonat / 콘팡"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-bug", "type-poison", "evo-line-venonat"]
aliases: ["콘팡", "venonat"]
summary: "#048 콘팡 (벌레 / 독) — GO 최대 CP: 1,004"
evolution_line: "venonat"
---

# Venonat / 콘팡

## Definition / 정의
**콘팡** (#048)는 일반 포켓몬로, [[type-bug|벌레]], [[type-poison|독]] 타입입니다.
"작은 눈이 많이 모여 큰 눈을 이루고 있다. 밤이 되면 불빛을 향해 모여든다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #048 |
| 영문 이름 | Venonat |
| 타입 | [[type-bug|벌레]], [[type-poison|독]] |
| 키 | 1.0m |
| 몸무게 | 30.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 190 |


## 도감 설명

> 작은 눈이 많이 모여 큰 눈을 이루고 있다. 밤이 되면 불빛을 향해 모여든다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 60 |
| 공격 | 55 |
| 방어 | 50 |
| 특수공격 | 40 |
| 특수방어 | 55 |
| 스피드 | 45 |
| **합계** | **305** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-venomoth|도나리]] |
| 계열 루트 | [[pokemon-venonat]] |

## 특성 (Abilities)
- compound-eyes
- tinted-lens
- run-away

## Related Concepts / 관련 개념
- [[type-bug|벌레 타입]] — 콘팡의 타입
- [[type-poison|독 타입]] — 콘팡의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 100 |
| 방어 (GO) | 100 |
| 체력 (GO) | 155 |
| 최대 CP (Lv.40) | 1,004 |
| 최대 CP (Lv.50) | 1,135 |
| 타입 | 벌레 / 독 |
| 버디 거리 | 3km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 비행, 바위, 불꽃, 에스퍼 |
| 저항 | 격투(×¼), 독, 벌레, 풀(×¼), 페어리 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-bug-bite|벌레먹음]], [[move-confusion|염동력]]
- **스페셜 기술:** [[move-poison-fang|독엄니]], [[move-psybeam|환상빔]], [[move-signal-beam|시그널빔]]

### 진화 체인
- **진화 후:** [[pokemon-venomoth|도나리]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| VENONAT_SHADOW | 100 | 100 | 155 | 1,004 |
## References
- Source: PokéAPI v2 pokemon/48
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
