---
title: "Machamp / 괴력몬"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-fighting", "evo-line-machop"]
aliases: ["괴력몬", "machamp"]
summary: "#068 괴력몬 (격투) — GO 최대 CP: 3,056"
evolution_line: "machop"
---

# Machamp / 괴력몬

## Definition / 정의
**괴력몬** (#068)는 일반 포켓몬로, [[type-fighting|격투]] 타입입니다.
"발달한 4개의 팔은 2초 동안 1000번의 펀치를 날릴 수 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #068 |
| 영문 이름 | Machamp |
| 타입 | [[type-fighting|격투]] |
| 키 | 1.6m |
| 몸무게 | 130.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |


## 도감 설명

> 발달한 4개의 팔은 2초 동안 1000번의 펀치를 날릴 수 있다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 90 |
| 공격 | 130 |
| 방어 | 80 |
| 특수공격 | 65 |
| 특수방어 | 85 |
| 스피드 | 55 |
| **합계** | **505** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-machoke|근육몬]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-machop]] |

## 특성 (Abilities)
- guts
- no-guard
- steadfast

## Related Concepts / 관련 개념
- [[type-fighting|격투 타입]] — 괴력몬의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 234 |
| 방어 (GO) | 159 |
| 체력 (GO) | 207 |
| 최대 CP (Lv.40) | 3,056 |
| 최대 CP (Lv.50) | 3,455 |
| 타입 | 격투 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|237위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|238위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|161위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 비행, 에스퍼, 페어리 |
| 저항 | 바위, 벌레, 악 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-bullet-punch|불릿펀치]], [[move-counter|카운터]], [[move-karate-chop|태권당수]]
- **스페셜 기술:** [[move-close-combat|인파이트]], [[move-cross-chop|크로스촙]], [[move-dynamic-punch|폭발펀치]], [[move-heavy-slam|헤비봄버]], [[move-rock-slide|스톤샤워]], [[move-stone-edge|스톤에지]], [[move-submission|지옥의바퀴]], [[move-payback|보복]]  *(커뮤니티 데이/전용: [[move-karate-chop|태권당수]], [[move-stone-edge|스톤에지]], [[move-submission|지옥의바퀴]], [[move-payback|보복]])*

### 진화 체인
- **진화 전:** [[pokemon-machoke|근육몬]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| MACHAMP_SHADOW | 234 | 159 | 207 | 3,056 |
## References
- Source: PokéAPI v2 pokemon/68
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
