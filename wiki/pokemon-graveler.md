---
title: "Graveler / 데구리"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-rock", "type-ground", "evo-line-geodude"]
aliases: ["데구리", "graveler"]
summary: "#075 데구리 (바위 / 땅) — GO 최대 CP: 1,897"
evolution_line: "geodude"
---

# Graveler / 데구리

## Definition / 정의
**데구리** (#075)는 일반 포켓몬로, [[type-rock|바위]], [[type-ground|땅]] 타입입니다.
"산에서 굴러 떨어질 때 몸의 여기저기가 부서져도 신경 쓰지 않는 호쾌한 성격이다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #075 |
| 영문 이름 | Graveler |
| 타입 | [[type-rock|바위]], [[type-ground|땅]] |
| 키 | 1.0m |
| 몸무게 | 105.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 120 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 55 |
| 공격 | 95 |
| 방어 | 115 |
| 특수공격 | 45 |
| 특수방어 | 45 |
| 스피드 | 35 |
| **합계** | **390** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-geodude|꼬마돌]] |
| 진화 후 | [[pokemon-golem|딱구리]] |
| 계열 루트 | [[pokemon-geodude]] |

## 특성 (Abilities)
- rock-head
- sturdy
- sand-veil

## Related Concepts / 관련 개념
- [[type-rock|바위 타입]] — 데구리의 타입
- [[type-ground|땅 타입]] — 데구리의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 164 |
| 방어 (GO) | 164 |
| 체력 (GO) | 146 |
| 최대 CP (Lv.40) | 1,897 |
| 최대 CP (Lv.50) | 2,145 |
| 타입 | 바위 / 땅 |
| 버디 거리 | 1km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 격투, 땅, 강철, 물(×4), 풀(×4), 얼음 |
| 저항 | 노말, 비행, 독(×¼), 바위, 불꽃 |
| 무효 | 전기 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-mud-shot|머드숏]], [[move-mud-slap|진흙뿌리기]], [[move-rock-throw|돌떨구기]]
- **스페셜 기술:** [[move-dig|구멍파기]], [[move-rock-blast|락블레스트]], [[move-rock-slide|스톤샤워]], [[move-stone-edge|스톤에지]]  *(커뮤니티 데이/전용: [[move-rock-slide|스톤샤워]])*

### 진화 체인
- **진화 전:** [[pokemon-geodude|꼬마돌]]
- **진화 후:** [[pokemon-golem|딱구리]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| GRAVELER_ALOLAN | 164 | 164 | 146 | 1,897 |
| GRAVELER_ALOLAN_SHADOW | 164 | 164 | 146 | 1,897 |
| GRAVELER_SHADOW | 164 | 164 | 146 | 1,897 |
## References
- Source: PokéAPI v2 pokemon/75
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
