---
title: "Beedrill / 독침붕"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-bug", "type-poison", "evo-line-weedle"]
aliases: ["독침붕", "beedrill"]
summary: "#015 독침붕 (벌레/독 타입) — 기본 스탯 합계 395"
evolution_line: "weedle"
---

# Beedrill / 독침붕

## Definition / 정의
**독침붕** (#015)는 일반 포켓몬로, [[type-bug|벌레]], [[type-poison|독]] 타입입니다.
"양손과 엉덩이에 있는 3개의 독침으로 상대를 찌르고 찌르고 또 찌르며 공격한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #015 |
| 영문 이름 | Beedrill |
| 타입 | [[type-bug|벌레]], [[type-poison|독]] |
| 키 | 1.0m |
| 몸무게 | 29.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 65 |
| 공격 | 90 |
| 방어 | 40 |
| 특수공격 | 45 |
| 특수방어 | 80 |
| 스피드 | 75 |
| **합계** | **395** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-kakuna|딱충이]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-weedle]] |

## 특성 (Abilities)
- swarm
- sniper

## Related Concepts / 관련 개념
- [[type-bug|벌레 타입]] — 독침붕의 타입
- [[type-poison|독 타입]] — 독침붕의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 169 |
| 방어 (GO) | 130 |
| 체력 (GO) | 163 |
| 최대 CP (Lv.40) | 1,846 |
| 최대 CP (Lv.50) | 2,087 |
| 타입 | 벌레 / 독 |
| 버디 거리 | 1km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|453위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-bug-bite|벌레먹음]], [[move-infestation|엉겨붙기]], [[move-poison-jab|독찌르기]], [[move-poison-sting|독침]]
- **스페셜 기술:** [[move-aerial-ace|제비반환]], [[move-sludge-bomb|오물폭탄]], X Scissor, [[move-fell-stinger|마지막일침]], [[move-drill-run|드릴라이너]]  *(커뮤니티 데이/전용: [[move-bug-bite|벌레먹음]], [[move-drill-run|드릴라이너]])*

### 진화 체인
- **진화 전:** [[pokemon-kakuna|딱충이]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| BEEDRILL_MEGA | 303 | 148 | 163 | 3,383 |
| BEEDRILL_SHADOW | 169 | 130 | 163 | 1,846 |
## References
- Source: PokéAPI v2 pokemon/15
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
