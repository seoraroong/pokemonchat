---
title: "Articuno / 프리져"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-ice", "type-flying", "evo-line-articuno", "legendary"]
aliases: ["프리져", "articuno"]
summary: "#144 프리져 (얼음 / 비행) — GO 최대 CP: 3,051"
evolution_line: "articuno"
---

# Articuno / 프리져

## Definition / 정의
**프리져** (#144)는 전설 포켓몬로, [[type-ice|얼음]], [[type-flying|비행]] 타입입니다.
"전설의 새포켓몬이다. 공기 중의 수분을 얼려 눈보라를 만들어 낼 수 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #144 |
| 영문 이름 | Articuno |
| 타입 | [[type-ice|얼음]], [[type-flying|비행]] |
| 키 | 1.7m |
| 몸무게 | 55.4kg |
| 분류 | 전설 포켓몬 |
| 포획률 | 3 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 90 |
| 공격 | 85 |
| 방어 | 100 |
| 특수공격 | 95 |
| 특수방어 | 125 |
| 스피드 | 85 |
| **합계** | **580** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-articuno]] |

## 특성 (Abilities)
- pressure
- snow-cloak

## Related Concepts / 관련 개념
- [[type-ice|얼음 타입]] — 프리져의 타입
- [[type-flying|비행 타입]] — 프리져의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 192 |
| 방어 (GO) | 236 |
| 체력 (GO) | 207 |
| 최대 CP (Lv.40) | 3,051 |
| 최대 CP (Lv.50) | 3,450 |
| 타입 | 얼음 / 비행 |
| 버디 거리 | 20km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|197위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|103위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|125위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 바위(×4), 강철, 불꽃, 전기 |
| 저항 | 벌레, 풀 |
| 무효 | 땅 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-frost-breath|얼음숨결]], [[move-ice-shard|얼음뭉치]], [[move-powder-snow|눈싸라기]]
- **스페셜 기술:** [[move-blizzard|눈보라]], [[move-hurricane|폭풍]], [[move-ice-beam|냉동빔]], [[move-icy-wind|얼다바람]], [[move-ancient-power|원시의힘]], [[move-triple-axel|트리플악셀]]  *(커뮤니티 데이/전용: [[move-hurricane|폭풍]])*

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| ARTICUNO_GALARIAN | 250 | 197 | 207 | 3,590 |
| ARTICUNO_SHADOW | 192 | 236 | 207 | 3,051 |
## References
- Source: PokéAPI v2 pokemon/144
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
