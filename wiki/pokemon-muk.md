---
title: "Muk / 질뻐기"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-poison", "evo-line-grimer"]
aliases: ["질뻐기", "muk"]
summary: "#089 질뻐기 (독) — GO 최대 CP: 2,757"
evolution_line: "grimer"
---

# Muk / 질뻐기

## Definition / 정의
**질뻐기** (#089)는 일반 포켓몬로, [[type-poison|독]] 타입입니다.
"진흙이 쌓여 악취가 나는 장소를 좋아해서 모여들기 때문에 주변은 더욱 악취가 난다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #089 |
| 영문 이름 | Muk |
| 타입 | [[type-poison|독]] |
| 키 | 1.2m |
| 몸무게 | 30.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 75 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 105 |
| 공격 | 105 |
| 방어 | 75 |
| 특수공격 | 65 |
| 특수방어 | 100 |
| 스피드 | 50 |
| **합계** | **500** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-grimer|질퍽이]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-grimer]] |

## 특성 (Abilities)
- stench
- sticky-hold
- poison-touch

## Related Concepts / 관련 개념
- [[type-poison|독 타입]] — 질뻐기의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 190 |
| 방어 (GO) | 172 |
| 체력 (GO) | 233 |
| 최대 CP (Lv.40) | 2,757 |
| 최대 CP (Lv.50) | 3,117 |
| 타입 | 독 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|473위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|350위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|335위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅, 에스퍼 |
| 저항 | 격투, 독, 벌레, 풀, 페어리 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-acid|용해액]], [[move-infestation|엉겨붙기]], [[move-lick|핥기]], [[move-poison-jab|독찌르기]]
- **스페셜 기술:** [[move-dark-pulse|악의파동]], [[move-gunk-shot|더스트슈트]], [[move-sludge-wave|오물웨이브]], [[move-thunder-punch|번개펀치]], [[move-acid-spray|애시드봄]]  *(커뮤니티 데이/전용: [[move-lick|핥기]])*

### 진화 체인
- **진화 전:** [[pokemon-grimer|질퍽이]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| MUK_ALOLAN | 190 | 172 | 233 | 2,757 |
| MUK_ALOLAN_SHADOW | 190 | 172 | 233 | 2,757 |
| MUK_SHADOW | 190 | 172 | 233 | 2,757 |
## References
- Source: PokéAPI v2 pokemon/89
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
