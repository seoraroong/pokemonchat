---
title: "Krabby / 크랩"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-water", "evo-line-krabby"]
aliases: ["크랩", "krabby"]
summary: "#098 크랩 (물) — GO 최대 CP: 1,561"
evolution_line: "krabby"
---

# Krabby / 크랩

## Definition / 정의
**크랩** (#098)는 일반 포켓몬로, [[type-water|물]] 타입입니다.
"위험이 닥치면 입에서 뿜어내는 거품으로 전신을 감싸서 몸을 크게 보이려고 한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #098 |
| 영문 이름 | Krabby |
| 타입 | [[type-water|물]] |
| 키 | 0.4m |
| 몸무게 | 6.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 225 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 30 |
| 공격 | 105 |
| 방어 | 90 |
| 특수공격 | 25 |
| 특수방어 | 25 |
| 스피드 | 50 |
| **합계** | **325** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-kingler|킹크랩]] |
| 계열 루트 | [[pokemon-krabby]] |

## 특성 (Abilities)
- hyper-cutter
- shell-armor
- sheer-force

## Related Concepts / 관련 개념
- [[type-water|물 타입]] — 크랩의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 181 |
| 방어 (GO) | 124 |
| 체력 (GO) | 102 |
| 최대 CP (Lv.40) | 1,561 |
| 최대 CP (Lv.50) | 1,764 |
| 타입 | 물 |
| 버디 거리 | 3km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 풀, 전기 |
| 저항 | 강철, 불꽃, 물, 얼음 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-bubble|거품]], [[move-mud-shot|머드숏]]
- **스페셜 기술:** [[move-bubble-beam|거품광선]], Vice Grip, [[move-water-pulse|물의파동]], [[move-razor-shell|셸블레이드]]

### 진화 체인
- **진화 후:** [[pokemon-kingler|킹크랩]]
## References
- Source: PokéAPI v2 pokemon/98
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
