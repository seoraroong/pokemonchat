---
title: "Pikachu / 피카츄"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-electric", "evo-line-pikachu"]
aliases: ["피카츄", "pikachu"]
summary: "#025 피카츄 (전기) — GO 최대 CP: 938"
evolution_line: "pikachu"
---

# Pikachu / 피카츄

## Definition / 정의
**피카츄** (#025)는 일반 포켓몬로, [[type-electric|전기]] 타입입니다.
"꼬리를 세우고 주변의 상황을 살피다 보면 가끔 꼬리에 번개가 친다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #025 |
| 영문 이름 | Pikachu |
| 타입 | [[type-electric|전기]] |
| 키 | 0.4m |
| 몸무게 | 6.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 190 |


## 도감 설명

> 꼬리를 세우고 주변의 상황을 살피다 보면 가끔 꼬리에 번개가 친다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 35 |
| 공격 | 55 |
| 방어 | 40 |
| 특수공격 | 50 |
| 특수방어 | 50 |
| 스피드 | 90 |
| **합계** | **320** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-raichu|라이츄]] |
| 계열 루트 | [[pokemon-pikachu]] |

## 특성 (Abilities)
- static
- lightning-rod

## Related Concepts / 관련 개념
- [[type-electric|전기 타입]] — 피카츄의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 112 |
| 방어 (GO) | 96 |
| 체력 (GO) | 111 |
| 최대 CP (Lv.40) | 938 |
| 최대 CP (Lv.50) | 1,060 |
| 타입 | 전기 |
| 버디 거리 | 1km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅 |
| 저항 | 비행, 강철, 전기 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-present|프레젠트]], [[move-quick-attack|전광석화]], [[move-thunder-shock|전기쇼크]]
- **스페셜 기술:** [[move-discharge|방전]], [[move-surf|파도타기]], [[move-thunder|번개]], [[move-thunderbolt|10만볼트]], [[move-wild-charge|와일드볼트]]  *(커뮤니티 데이/전용: [[move-present|프레젠트]], [[move-surf|파도타기]], [[move-thunder|번개]])*

### 진화 체인
- **진화 전:** [[pokemon-pichu|피츄]]
- **진화 후:** [[pokemon-raichu|라이츄]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| PIKACHU_LIBRE | 112 | 96 | 111 | 938 |
| PIKACHU_POP_STAR | 112 | 96 | 111 | 938 |
| PIKACHU_ROCK_STAR | 112 | 96 | 111 | 938 |
## References
- Source: PokéAPI v2 pokemon/25
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
