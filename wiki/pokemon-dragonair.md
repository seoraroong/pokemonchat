---
title: "Dragonair / 신뇽"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-dragon", "evo-line-dratini"]
aliases: ["신뇽", "dragonair"]
summary: "#148 신뇽 (드래곤) — GO 최대 CP: 1,780"
evolution_line: "dratini"
---

# Dragonair / 신뇽

## Definition / 정의
**신뇽** (#148)는 일반 포켓몬로, [[type-dragon|드래곤]] 타입입니다.
"수정 같은 구슬에는 날씨를 자유롭게 조종하는 능력이 담겨 있는 듯하다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #148 |
| 영문 이름 | Dragonair |
| 타입 | [[type-dragon|드래곤]] |
| 키 | 4.0m |
| 몸무게 | 16.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |


## 도감 설명

> 수정 같은 구슬에는 날씨를 자유롭게 조종하는 능력이 담겨 있는 듯하다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 61 |
| 공격 | 84 |
| 방어 | 65 |
| 특수공격 | 70 |
| 특수방어 | 70 |
| 스피드 | 70 |
| **합계** | **420** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-dratini|미뇽]] |
| 진화 후 | [[pokemon-dragonite|망나뇽]] |
| 계열 루트 | [[pokemon-dratini]] |

## 특성 (Abilities)
- shed-skin
- marvel-scale

## Related Concepts / 관련 개념
- [[type-dragon|드래곤 타입]] — 신뇽의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 163 |
| 방어 (GO) | 135 |
| 체력 (GO) | 156 |
| 최대 CP (Lv.40) | 1,780 |
| 최대 CP (Lv.50) | 2,012 |
| 타입 | 드래곤 |
| 버디 거리 | 5km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|93위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 얼음, 드래곤, 페어리 |
| 저항 | 불꽃, 물, 풀, 전기 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-dragon-breath|용의숨결]], [[move-iron-tail|아이언테일]]
- **스페셜 기술:** [[move-aqua-tail|아쿠아테일]], [[move-dragon-pulse|용의파동]], [[move-wrap|김밥말이]], [[move-body-slam|누르기]]

### 진화 체인
- **진화 전:** [[pokemon-dratini|미뇽]]
- **진화 후:** [[pokemon-dragonite|망나뇽]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| DRAGONAIR_SHADOW | 163 | 135 | 156 | 1,780 |
## References
- Source: PokéAPI v2 pokemon/148
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
