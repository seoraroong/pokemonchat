---
title: "Hitmonchan / 홍수몬"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-fighting", "evo-line-hitmonchan"]
aliases: ["홍수몬", "hitmonchan"]
summary: "#107 홍수몬 (격투) — GO 최대 CP: 2,332"
evolution_line: "hitmonchan"
---

# Hitmonchan / 홍수몬

## Definition / 정의
**홍수몬** (#107)는 일반 포켓몬로, [[type-fighting|격투]] 타입입니다.
"팔을 비틀며 날리는 펀치는 콘크리트도 부스러뜨린다. 3분 싸우면 잠시 쉰다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #107 |
| 영문 이름 | Hitmonchan |
| 타입 | [[type-fighting|격투]] |
| 키 | 1.4m |
| 몸무게 | 50.2kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 50 |
| 공격 | 105 |
| 방어 | 79 |
| 특수공격 | 35 |
| 특수방어 | 110 |
| 스피드 | 76 |
| **합계** | **455** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-hitmonchan]] |

## 특성 (Abilities)
- keen-eye
- iron-fist
- inner-focus

## Related Concepts / 관련 개념
- [[type-fighting|격투 타입]] — 홍수몬의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 193 |
| 방어 (GO) | 197 |
| 체력 (GO) | 137 |
| 최대 CP (Lv.40) | 2,332 |
| 최대 CP (Lv.50) | 2,636 |
| 타입 | 격투 |
| 버디 거리 | 5km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 비행, 에스퍼, 페어리 |
| 저항 | 바위, 벌레, 악 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-bullet-punch|불릿펀치]], [[move-counter|카운터]], [[move-rock-smash|바위깨기]]
- **스페셜 기술:** [[move-brick-break|깨트리다]], [[move-close-combat|인파이트]], [[move-fire-punch|불꽃펀치]], [[move-ice-punch|냉동펀치]], [[move-thunder-punch|번개펀치]], Power Up Punch  *(커뮤니티 데이/전용: [[move-rock-smash|바위깨기]], [[move-brick-break|깨트리다]])*

### 진화 체인
- **진화 전:** [[pokemon-tyrogue|배루키]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| HITMONCHAN_SHADOW | 193 | 197 | 137 | 2,332 |
## References
- Source: PokéAPI v2 pokemon/107
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
