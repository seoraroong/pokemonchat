---
title: "Vileplume / 라플레시아"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-grass", "type-poison", "evo-line-oddish"]
aliases: ["라플레시아", "vileplume"]
summary: "#045 라플레시아 (풀/독 타입) — 기본 스탯 합계 490"
evolution_line: "oddish"
---

# Vileplume / 라플레시아

## Definition / 정의
**라플레시아** (#045)는 일반 포켓몬로, [[type-grass|풀]], [[type-poison|독]] 타입입니다.
"꽃잎이 클수록 많은 꽃가루를 만들어 내지만 머리가 무거워서 지쳐버린다고 한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #045 |
| 영문 이름 | Vileplume |
| 타입 | [[type-grass|풀]], [[type-poison|독]] |
| 키 | 1.2m |
| 몸무게 | 18.6kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 75 |
| 공격 | 80 |
| 방어 | 85 |
| 특수공격 | 110 |
| 특수방어 | 90 |
| 스피드 | 50 |
| **합계** | **490** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-gloom|냄새꼬]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-oddish]] |

## 특성 (Abilities)
- chlorophyll
- effect-spore

## Related Concepts / 관련 개념
- [[type-grass|풀 타입]] — 라플레시아의 타입
- [[type-poison|독 타입]] — 라플레시아의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 202 |
| 방어 (GO) | 167 |
| 체력 (GO) | 181 |
| 최대 CP (Lv.40) | 2,559 |
| 최대 CP (Lv.50) | 2,893 |
| 타입 | 풀 / 독 |
| 버디 거리 | 3km |

### 기술 풀 (GO)
- **빠른 기술:** [[move-acid|용해액]], [[move-razor-leaf|잎날가르기]]
- **스페셜 기술:** [[move-moonblast|문포스]], [[move-petal-blizzard|꽃보라]], [[move-solar-beam|솔라빔]], [[move-sludge-bomb|오물폭탄]]

### 진화 체인
- **진화 전:** [[pokemon-gloom|냄새꼬]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| VILEPLUME_SHADOW | 202 | 167 | 181 | 2,559 |
## References
- Source: PokéAPI v2 pokemon/45
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
