---
title: "Gengar / 팬텀"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-ghost", "type-poison", "evo-line-gastly"]
aliases: ["팬텀", "gengar"]
summary: "#094 팬텀 (고스트/독 타입) — 기본 스탯 합계 500"
evolution_line: "gastly"
---

# Gengar / 팬텀

## Definition / 정의
**팬텀** (#094)는 일반 포켓몬로, [[type-ghost|고스트]], [[type-poison|독]] 타입입니다.
"그림자에 모습을 숨긴다. 팬텀이 숨어 있는 방은 온도가 5도 내려간다고 전해진다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #094 |
| 영문 이름 | Gengar |
| 타입 | [[type-ghost|고스트]], [[type-poison|독]] |
| 키 | 1.5m |
| 몸무게 | 40.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 60 |
| 공격 | 65 |
| 방어 | 60 |
| 특수공격 | 130 |
| 특수방어 | 75 |
| 스피드 | 110 |
| **합계** | **500** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-haunter|고우스트]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-gastly]] |

## 특성 (Abilities)
- cursed-body

## Related Concepts / 관련 개념
- [[type-ghost|고스트 타입]] — 팬텀의 타입
- [[type-poison|독 타입]] — 팬텀의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 261 |
| 방어 (GO) | 149 |
| 체력 (GO) | 155 |
| 최대 CP (Lv.40) | 2,878 |
| 최대 CP (Lv.50) | 3,254 |
| 타입 | 고스트 / 독 |
| 버디 거리 | 3km |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|332위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-hex|병상첨병]], [[move-lick|핥기]], [[move-shadow-claw|섀도크루]], [[move-sucker-punch|기습]]
- **스페셜 기술:** [[move-dark-pulse|악의파동]], [[move-focus-blast|기합구슬]], [[move-psychic|사이코키네시스]], [[move-shadow-ball|섀도볼]], [[move-sludge-bomb|오물폭탄]], [[move-sludge-wave|오물웨이브]], [[move-shadow-punch|섀도펀치]], [[move-drain-punch|드레인펀치]]  *(커뮤니티 데이/전용: [[move-lick|핥기]], [[move-sludge-wave|오물웨이브]], [[move-dark-pulse|악의파동]], [[move-psychic|사이코키네시스]], [[move-shadow-punch|섀도펀치]])*

### 진화 체인
- **진화 전:** [[pokemon-haunter|고우스트]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| GENGAR_MEGA | 349 | 199 | 155 | 4,336 |
| GENGAR_SHADOW | 261 | 149 | 155 | 2,878 |
## References
- Source: PokéAPI v2 pokemon/94
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
