---
title: "Kabuto / 투구"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-rock", "type-water", "evo-line-kabuto"]
aliases: ["투구", "kabuto"]
summary: "#140 투구 (바위/물 타입) — 기본 스탯 합계 355"
evolution_line: "kabuto"
---

# Kabuto / 투구

## Definition / 정의
**투구** (#140)는 일반 포켓몬로, [[type-rock|바위]], [[type-water|물]] 타입입니다.
"3억 년 전에 모래 해변에서 살고 있었던 것으로 추측된다. 단단한 껍질이 몸을 보호한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #140 |
| 영문 이름 | Kabuto |
| 타입 | [[type-rock|바위]], [[type-water|물]] |
| 키 | 0.5m |
| 몸무게 | 11.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 30 |
| 공격 | 80 |
| 방어 | 90 |
| 특수공격 | 55 |
| 특수방어 | 45 |
| 스피드 | 55 |
| **합계** | **355** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-kabutops|투구푸스]] |
| 계열 루트 | [[pokemon-kabuto]] |

## 특성 (Abilities)
- swift-swim
- battle-armor
- weak-armor

## Related Concepts / 관련 개념
- [[type-rock|바위 타입]] — 투구의 타입
- [[type-water|물 타입]] — 투구의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 148 |
| 방어 (GO) | 140 |
| 체력 (GO) | 102 |
| 최대 CP (Lv.40) | 1,370 |
| 최대 CP (Lv.50) | 1,549 |
| 타입 | 바위 / 물 |
| 버디 거리 | 5km |

### 기술 풀 (GO)
- **빠른 기술:** [[move-mud-shot|머드숏]], [[move-scratch|할퀴기]]
- **스페셜 기술:** [[move-ancient-power|원시의힘]], [[move-aqua-jet|아쿠아제트]], [[move-rock-tomb|암석봉인]]

### 진화 체인
- **진화 후:** [[pokemon-kabutops|투구푸스]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| KABUTO_SHADOW | 148 | 140 | 102 | 1,370 |
## References
- Source: PokéAPI v2 pokemon/140
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
