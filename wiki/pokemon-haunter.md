---
title: "Haunter / 고우스트"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-ghost", "type-poison", "evo-line-gastly"]
aliases: ["고우스트", "haunter"]
summary: "#093 고우스트 (고스트 / 독) — GO 최대 CP: 1,963"
evolution_line: "gastly"
---

# Haunter / 고우스트

## Definition / 정의
**고우스트** (#093)는 일반 포켓몬로, [[type-ghost|고스트]], [[type-poison|독]] 타입입니다.
"어둠 속에서 아무도 없는데도 누군가가 보고 있다는 느낌이 들면 그곳에 고우스트가 있는 것이다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #093 |
| 영문 이름 | Haunter |
| 타입 | [[type-ghost|고스트]], [[type-poison|독]] |
| 키 | 1.6m |
| 몸무게 | 0.1kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 90 |


## 도감 설명

> 어둠 속에서 아무도 없는데도 누군가가 보고 있다는 느낌이 들면 그곳에 고우스트가 있는 것이다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 45 |
| 공격 | 50 |
| 방어 | 45 |
| 특수공격 | 115 |
| 특수방어 | 55 |
| 스피드 | 95 |
| **합계** | **405** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-gastly|고오스]] |
| 진화 후 | [[pokemon-gengar|팬텀]] |
| 계열 루트 | [[pokemon-gastly]] |

## 특성 (Abilities)
- levitate

## Related Concepts / 관련 개념
- [[type-ghost|고스트 타입]] — 고우스트의 타입
- [[type-poison|독 타입]] — 고우스트의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 223 |
| 방어 (GO) | 107 |
| 체력 (GO) | 128 |
| 최대 CP (Lv.40) | 1,963 |
| 최대 CP (Lv.50) | 2,219 |
| 타입 | 고스트 / 독 |
| 버디 거리 | 3km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅, 고스트, 에스퍼, 악 |
| 저항 | 독(×¼), 벌레(×¼), 풀, 페어리 |
| 무효 | 노말, 격투 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-astonish|놀래키기]], [[move-lick|핥기]], [[move-shadow-claw|섀도크루]]
- **스페셜 기술:** [[move-dark-pulse|악의파동]], [[move-shadow-ball|섀도볼]], [[move-shadow-punch|섀도펀치]], [[move-sludge-bomb|오물폭탄]], [[move-ice-punch|냉동펀치]]  *(커뮤니티 데이/전용: [[move-lick|핥기]])*

### 진화 체인
- **진화 전:** [[pokemon-gastly|고오스]]
- **진화 후:** [[pokemon-gengar|팬텀]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| HAUNTER_SHADOW | 223 | 107 | 128 | 1,963 |
## References
- Source: PokéAPI v2 pokemon/93
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
