---
title: "Nidoqueen / 니드퀸"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-poison", "type-ground", "evo-line-nidoran-f"]
aliases: ["니드퀸", "nidoqueen"]
summary: "#031 니드퀸 (독 / 땅) — GO 최대 CP: 2,488"
evolution_line: "nidoran-f"
---

# Nidoqueen / 니드퀸

## Definition / 정의
**니드퀸** (#031)는 일반 포켓몬로, [[type-poison|독]], [[type-ground|땅]] 타입입니다.
"비늘로 뒤덮인 튼튼한 몸으로 둥지의 입구를 막아 상대로부터 새끼들을 지킨다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #031 |
| 영문 이름 | Nidoqueen |
| 타입 | [[type-poison|독]], [[type-ground|땅]] |
| 키 | 1.3m |
| 몸무게 | 60.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 90 |
| 공격 | 92 |
| 방어 | 87 |
| 특수공격 | 75 |
| 특수방어 | 85 |
| 스피드 | 76 |
| **합계** | **505** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-nidorina|니드리나]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-nidoran-f]] |

## 특성 (Abilities)
- poison-point
- rivalry
- sheer-force

## Related Concepts / 관련 개념
- [[type-poison|독 타입]] — 니드퀸의 타입
- [[type-ground|땅 타입]] — 니드퀸의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 180 |
| 방어 (GO) | 173 |
| 체력 (GO) | 207 |
| 최대 CP (Lv.40) | 2,488 |
| 최대 CP (Lv.50) | 2,812 |
| 타입 | 독 / 땅 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|227위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|107위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅, 물, 에스퍼, 얼음 |
| 저항 | 격투, 독(×¼), 바위, 벌레, 페어리 |
| 무효 | 전기 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-bite|물기]], [[move-poison-jab|독찌르기]], [[move-poison-sting|독침]], [[move-mud-slap|진흙뿌리기]]
- **스페셜 기술:** [[move-earthquake|지진]], [[move-sludge-wave|오물웨이브]], [[move-stone-edge|스톤에지]], [[move-earth-power|대지의힘]], [[move-poison-fang|독엄니]]

### 진화 체인
- **진화 전:** [[pokemon-nidorina|니드리나]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| NIDOQUEEN_SHADOW | 180 | 173 | 207 | 2,488 |
## References
- Source: PokéAPI v2 pokemon/31
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
