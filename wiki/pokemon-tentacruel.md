---
title: "Tentacruel / 독파리"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-water", "type-poison", "evo-line-tentacool"]
aliases: ["독파리", "tentacruel"]
summary: "#073 독파리 (물 / 독) — GO 최대 CP: 2,422"
evolution_line: "tentacool"
---

# Tentacruel / 독파리

## Definition / 정의
**독파리** (#073)는 일반 포켓몬로, [[type-water|물]], [[type-poison|독]] 타입입니다.
"촉수는 평소에는 짧고 먹이를 찌를 때 길게 늘어나 휘감는다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #073 |
| 영문 이름 | Tentacruel |
| 타입 | [[type-water|물]], [[type-poison|독]] |
| 키 | 1.6m |
| 몸무게 | 55.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 60 |


## 도감 설명

> 촉수는 평소에는 짧고 먹이를 찌를 때 길게 늘어나 휘감는다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 80 |
| 공격 | 70 |
| 방어 | 65 |
| 특수공격 | 80 |
| 특수방어 | 120 |
| 스피드 | 100 |
| **합계** | **515** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-tentacool|왕눈해]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-tentacool]] |

## 특성 (Abilities)
- clear-body
- liquid-ooze
- rain-dish

## Related Concepts / 관련 개념
- [[type-water|물 타입]] — 독파리의 타입
- [[type-poison|독 타입]] — 독파리의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 166 |
| 방어 (GO) | 209 |
| 체력 (GO) | 190 |
| 최대 CP (Lv.40) | 2,422 |
| 최대 CP (Lv.50) | 2,738 |
| 타입 | 물 / 독 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|199위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|69위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|310위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅, 전기, 에스퍼 |
| 저항 | 격투, 독, 벌레, 강철, 불꽃, 물, 얼음, 페어리 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-acid|용해액]], [[move-poison-jab|독찌르기]], [[move-poison-sting|독침]]
- **스페셜 기술:** [[move-blizzard|눈보라]], [[move-hydro-pump|하이드로펌프]], [[move-sludge-wave|오물웨이브]], [[move-acid-spray|애시드봄]], [[move-scald|열탕]], [[move-payback|보복]]

### 진화 체인
- **진화 전:** [[pokemon-tentacool|왕눈해]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| TENTACRUEL_SHADOW | 166 | 209 | 190 | 2,422 |
## References
- Source: PokéAPI v2 pokemon/73
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
