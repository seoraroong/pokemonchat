---
title: "Onix / 롱스톤"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-rock", "type-ground", "evo-line-onix"]
aliases: ["롱스톤", "onix"]
summary: "#095 롱스톤 (바위 / 땅) — GO 최대 CP: 1,101"
evolution_line: "onix"
---

# Onix / 롱스톤

## Definition / 정의
**롱스톤** (#095)는 일반 포켓몬로, [[type-rock|바위]], [[type-ground|땅]] 타입입니다.
"땅속을 엄청난 기세로 뚫고 나아가며 먹이를 찾는다. 지나간 곳은 디그다의 보금자리가 된다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #095 |
| 영문 이름 | Onix |
| 타입 | [[type-rock|바위]], [[type-ground|땅]] |
| 키 | 8.8m |
| 몸무게 | 210.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |


## 도감 설명

> 땅속을 엄청난 기세로 뚫고 나아가며 먹이를 찾는다. 지나간 곳은 디그다의 보금자리가 된다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 35 |
| 공격 | 45 |
| 방어 | 160 |
| 특수공격 | 30 |
| 특수방어 | 45 |
| 스피드 | 70 |
| **합계** | **385** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-onix]] |

## 특성 (Abilities)
- rock-head
- sturdy
- weak-armor

## Related Concepts / 관련 개념
- [[type-rock|바위 타입]] — 롱스톤의 타입
- [[type-ground|땅 타입]] — 롱스톤의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 85 |
| 방어 (GO) | 232 |
| 체력 (GO) | 111 |
| 최대 CP (Lv.40) | 1,101 |
| 최대 CP (Lv.50) | 1,245 |
| 타입 | 바위 / 땅 |
| 버디 거리 | 5km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 격투, 땅, 강철, 물(×4), 풀(×4), 얼음 |
| 저항 | 노말, 비행, 독(×¼), 바위, 불꽃 |
| 무효 | 전기 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-rock-throw|돌떨구기]], [[move-tackle|몸통박치기]]
- **스페셜 기술:** [[move-heavy-slam|헤비봄버]], [[move-iron-head|아이언헤드]], [[move-rock-slide|스톤샤워]], [[move-sand-tomb|모래지옥]], [[move-stone-edge|스톤에지]], [[move-breaking-swipe|와이드브레이커]]  *(커뮤니티 데이/전용: [[move-iron-head|아이언헤드]], [[move-rock-slide|스톤샤워]])*

### 진화 체인
- **진화 후:** [[pokemon-steelix|강철톤]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| ONIX_SHADOW | 85 | 232 | 111 | 1,101 |
## References
- Source: PokéAPI v2 pokemon/95
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
