---
title: "Bulbasaur / 이상해씨"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-grass", "type-poison", "evo-line-bulbasaur"]
aliases: ["이상해씨", "bulbasaur"]
summary: "#001 이상해씨 (풀 / 독) — GO 최대 CP: 1,115"
evolution_line: "bulbasaur"
---

# Bulbasaur / 이상해씨

## Definition / 정의
**이상해씨** (#001)는 일반 포켓몬로, [[type-grass|풀]], [[type-poison|독]] 타입입니다.
"태어났을 때부터 등에 이상한 씨앗이 심어져 있으며 몸과 함께 자란다고 한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #001 |
| 영문 이름 | Bulbasaur |
| 타입 | [[type-grass|풀]], [[type-poison|독]] |
| 키 | 0.7m |
| 몸무게 | 6.9kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 45 |
| 공격 | 49 |
| 방어 | 49 |
| 특수공격 | 65 |
| 특수방어 | 65 |
| 스피드 | 45 |
| **합계** | **318** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-ivysaur|이상해풀]] |
| 계열 루트 | [[pokemon-bulbasaur]] |

## 특성 (Abilities)
- overgrow
- chlorophyll

## Related Concepts / 관련 개념
- [[type-grass|풀 타입]] — 이상해씨의 타입
- [[type-poison|독 타입]] — 이상해씨의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 118 |
| 방어 (GO) | 111 |
| 체력 (GO) | 128 |
| 최대 CP (Lv.40) | 1,115 |
| 최대 CP (Lv.50) | 1,260 |
| 타입 | 풀 / 독 |
| 버디 거리 | 3km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 비행, 불꽃, 에스퍼, 얼음 |
| 저항 | 격투, 물, 풀(×¼), 전기, 페어리 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-tackle|몸통박치기]], [[move-vine-whip|덩굴채찍]]
- **스페셜 기술:** [[move-power-whip|파워휩]], [[move-seed-bomb|씨폭탄]], [[move-sludge-bomb|오물폭탄]]

### 진화 체인
- **진화 후:** [[pokemon-ivysaur|이상해풀]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| BULBASAUR_SHADOW | 118 | 111 | 128 | 1,115 |
## References
- Source: PokéAPI v2 pokemon/1
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
