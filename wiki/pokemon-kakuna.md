---
title: "Kakuna / 딱충이"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-bug", "type-poison", "evo-line-weedle"]
aliases: ["딱충이", "kakuna"]
summary: "#014 딱충이 (벌레/독 타입) — 기본 스탯 합계 205"
evolution_line: "weedle"
---

# Kakuna / 딱충이

## Definition / 정의
**딱충이** (#014)는 일반 포켓몬로, [[type-bug|벌레]], [[type-poison|독]] 타입입니다.
"스스로는 거의 움직일 수 없지만 위험할 때는 단단해져서 몸을 보호하고 있는 것 같다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #014 |
| 영문 이름 | Kakuna |
| 타입 | [[type-bug|벌레]], [[type-poison|독]] |
| 키 | 0.6m |
| 몸무게 | 10.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 120 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 45 |
| 공격 | 25 |
| 방어 | 50 |
| 특수공격 | 25 |
| 특수방어 | 25 |
| 스피드 | 35 |
| **합계** | **205** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-weedle|뿔충이]] |
| 진화 후 | [[pokemon-beedrill|독침붕]] |
| 계열 루트 | [[pokemon-weedle]] |

## 특성 (Abilities)
- shed-skin

## Related Concepts / 관련 개념
- [[type-bug|벌레 타입]] — 딱충이의 타입
- [[type-poison|독 타입]] — 딱충이의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 46 |
| 방어 (GO) | 75 |
| 체력 (GO) | 128 |
| 최대 CP (Lv.40) | 432 |
| 최대 CP (Lv.50) | 488 |
| 타입 | 벌레 / 독 |
| 버디 거리 | 1km |

### 기술 풀 (GO)
- **빠른 기술:** [[move-bug-bite|벌레먹음]], [[move-poison-sting|독침]]
- **스페셜 기술:** [[move-struggle|발버둥]]

### 진화 체인
- **진화 전:** [[pokemon-weedle|뿔충이]]
- **진화 후:** [[pokemon-beedrill|독침붕]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| KAKUNA_SHADOW | 46 | 75 | 128 | 432 |
## References
- Source: PokéAPI v2 pokemon/14
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
