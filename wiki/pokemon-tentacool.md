---
title: "Tentacool / 왕눈해"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-water", "type-poison", "evo-line-tentacool"]
aliases: ["왕눈해", "tentacool"]
summary: "#072 왕눈해 (물 / 독) — GO 최대 CP: 1,040"
evolution_line: "tentacool"
---

# Tentacool / 왕눈해

## Definition / 정의
**왕눈해** (#072)는 일반 포켓몬로, [[type-water|물]], [[type-poison|독]] 타입입니다.
"몸 대부분이 수분이다. 수정 같은 눈에서 이상한 빔을 발사한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #072 |
| 영문 이름 | Tentacool |
| 타입 | [[type-water|물]], [[type-poison|독]] |
| 키 | 0.9m |
| 몸무게 | 45.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 190 |


## 도감 설명

> 몸 대부분이 수분이다. 수정 같은 눈에서 이상한 빔을 발사한다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 40 |
| 공격 | 40 |
| 방어 | 35 |
| 특수공격 | 50 |
| 특수방어 | 100 |
| 스피드 | 70 |
| **합계** | **335** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-tentacruel|독파리]] |
| 계열 루트 | [[pokemon-tentacool]] |

## 특성 (Abilities)
- clear-body
- liquid-ooze
- rain-dish

## Related Concepts / 관련 개념
- [[type-water|물 타입]] — 왕눈해의 타입
- [[type-poison|독 타입]] — 왕눈해의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 97 |
| 방어 (GO) | 149 |
| 체력 (GO) | 120 |
| 최대 CP (Lv.40) | 1,040 |
| 최대 CP (Lv.50) | 1,176 |
| 타입 | 물 / 독 |
| 버디 거리 | 3km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅, 전기, 에스퍼 |
| 저항 | 격투, 독, 벌레, 강철, 불꽃, 물, 얼음, 페어리 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-bubble|거품]], [[move-poison-sting|독침]]
- **스페셜 기술:** [[move-bubble-beam|거품광선]], [[move-water-pulse|물의파동]], [[move-wrap|김밥말이]]

### 진화 체인
- **진화 후:** [[pokemon-tentacruel|독파리]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| TENTACOOL_SHADOW | 97 | 149 | 120 | 1,040 |
## References
- Source: PokéAPI v2 pokemon/72
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
