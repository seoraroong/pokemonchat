---
title: "Omastar / 암스타"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-rock", "type-water", "evo-line-omanyte"]
aliases: ["암스타", "omastar"]
summary: "#139 암스타 (바위 / 물) — GO 최대 CP: 2,786"
evolution_line: "omanyte"
---

# Omastar / 암스타

## Definition / 정의
**암스타** (#139)는 일반 포켓몬로, [[type-rock|바위]], [[type-water|물]] 타입입니다.
"촉수가 팔다리처럼 발달해 있다. 달라붙자마자 문다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #139 |
| 영문 이름 | Omastar |
| 타입 | [[type-rock|바위]], [[type-water|물]] |
| 키 | 1.0m |
| 몸무게 | 35.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |


## 도감 설명

> 촉수가 팔다리처럼 발달해 있다. 달라붙자마자 문다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 70 |
| 공격 | 60 |
| 방어 | 125 |
| 특수공격 | 115 |
| 특수방어 | 70 |
| 스피드 | 55 |
| **합계** | **495** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-omanyte|암나이트]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-omanyte]] |

## 특성 (Abilities)
- swift-swim
- shell-armor
- weak-armor

## Related Concepts / 관련 개념
- [[type-rock|바위 타입]] — 암스타의 타입
- [[type-water|물 타입]] — 암스타의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 207 |
| 방어 (GO) | 201 |
| 체력 (GO) | 172 |
| 최대 CP (Lv.40) | 2,786 |
| 최대 CP (Lv.50) | 3,150 |
| 타입 | 바위 / 물 |
| 버디 거리 | 5km |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|366위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 격투, 땅, 풀(×4), 전기 |
| 저항 | 노말, 비행, 독, 불꽃(×¼), 얼음 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-mud-shot|머드숏]], [[move-rock-throw|돌떨구기]], [[move-water-gun|물대포]]
- **스페셜 기술:** [[move-ancient-power|원시의힘]], [[move-hydro-pump|하이드로펌프]], [[move-rock-blast|락블레스트]], [[move-rock-slide|스톤샤워]]  *(커뮤니티 데이/전용: [[move-rock-throw|돌떨구기]], [[move-rock-slide|스톤샤워]])*

### 진화 체인
- **진화 전:** [[pokemon-omanyte|암나이트]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| OMASTAR_SHADOW | 207 | 201 | 172 | 2,786 |
## References
- Source: PokéAPI v2 pokemon/139
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
