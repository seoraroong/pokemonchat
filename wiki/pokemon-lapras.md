---
title: "Lapras / 라프라스"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-water", "type-ice", "evo-line-lapras"]
aliases: ["라프라스", "lapras"]
summary: "#131 라프라스 (물 / 얼음) — GO 최대 CP: 2,641"
evolution_line: "lapras"
---

# Lapras / 라프라스

## Definition / 정의
**라프라스** (#131)는 일반 포켓몬로, [[type-water|물]], [[type-ice|얼음]] 타입입니다.
"부드러운 마음의 소유자다. 좀처럼 다투지 않아 많이 잡혔기 때문에 수가 줄었다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #131 |
| 영문 이름 | Lapras |
| 타입 | [[type-water|물]], [[type-ice|얼음]] |
| 키 | 2.5m |
| 몸무게 | 220.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |


## 도감 설명

> 부드러운 마음의 소유자다. 좀처럼 다투지 않아 많이 잡혔기 때문에 수가 줄었다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 130 |
| 공격 | 85 |
| 방어 | 80 |
| 특수공격 | 85 |
| 특수방어 | 95 |
| 스피드 | 60 |
| **합계** | **535** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-lapras]] |

## 특성 (Abilities)
- water-absorb
- shell-armor
- hydration

## Related Concepts / 관련 개념
- [[type-water|물 타입]] — 라프라스의 타입
- [[type-ice|얼음 타입]] — 라프라스의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 165 |
| 방어 (GO) | 174 |
| 체력 (GO) | 277 |
| 최대 CP (Lv.40) | 2,641 |
| 최대 CP (Lv.50) | 2,985 |
| 타입 | 물 / 얼음 |
| 버디 거리 | 5km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|32위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|28위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|199위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 격투, 바위, 풀, 전기 |
| 저항 | 물, 얼음(×¼) |
### 기술 풀 (GO)
- **빠른 기술:** [[move-frost-breath|얼음숨결]], [[move-ice-shard|얼음뭉치]], [[move-water-gun|물대포]], [[move-psywave|사이코웨이브]]
- **스페셜 기술:** [[move-blizzard|눈보라]], [[move-dragon-pulse|용의파동]], [[move-hydro-pump|하이드로펌프]], [[move-ice-beam|냉동빔]], [[move-surf|파도타기]], [[move-skull-bash|로케트박치기]], [[move-sparkling-aria|물거품아리아]]  *(커뮤니티 데이/전용: [[move-ice-shard|얼음뭉치]], [[move-dragon-pulse|용의파동]], [[move-ice-beam|냉동빔]])*

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| LAPRAS_SHADOW | 165 | 174 | 277 | 2,641 |
## References
- Source: PokéAPI v2 pokemon/131
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
