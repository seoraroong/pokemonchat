---
title: "Zapdos / 썬더"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-electric", "type-flying", "evo-line-zapdos", "legendary"]
aliases: ["썬더", "zapdos"]
summary: "#145 썬더 (전기 / 비행) — GO 최대 CP: 3,527"
evolution_line: "zapdos"
---

# Zapdos / 썬더

## Definition / 정의
**썬더** (#145)는 전설 포켓몬로, [[type-electric|전기]], [[type-flying|비행]] 타입입니다.
"구름 위에서 거대한 번개를 내리치며 나타난다. 전설의 새포켓몬이다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #145 |
| 영문 이름 | Zapdos |
| 타입 | [[type-electric|전기]], [[type-flying|비행]] |
| 키 | 1.6m |
| 몸무게 | 52.6kg |
| 분류 | 전설 포켓몬 |
| 포획률 | 3 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 90 |
| 공격 | 90 |
| 방어 | 85 |
| 특수공격 | 125 |
| 특수방어 | 90 |
| 스피드 | 100 |
| **합계** | **580** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-zapdos]] |

## 특성 (Abilities)
- pressure
- static

## Related Concepts / 관련 개념
- [[type-electric|전기 타입]] — 썬더의 타입
- [[type-flying|비행 타입]] — 썬더의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 253 |
| 방어 (GO) | 185 |
| 체력 (GO) | 207 |
| 최대 CP (Lv.40) | 3,527 |
| 최대 CP (Lv.50) | 3,987 |
| 타입 | 전기 / 비행 |
| 버디 거리 | 20km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|249위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|111위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|80위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 바위, 얼음 |
| 저항 | 격투, 비행, 벌레, 강철, 풀 |
| 무효 | 땅 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-charge-beam|차지빔]], [[move-thunder-shock|전기쇼크]]
- **스페셜 기술:** [[move-thunder|번개]], [[move-thunderbolt|10만볼트]], [[move-zap-cannon|전자포]], [[move-ancient-power|원시의힘]], [[move-drill-peck|회전부리]], [[move-heat-wave|열풍]]  *(커뮤니티 데이/전용: [[move-thunder-shock|전기쇼크]])*

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| ZAPDOS_GALARIAN | 252 | 189 | 207 | 3,548 |
| ZAPDOS_SHADOW | 253 | 185 | 207 | 3,527 |
## References
- Source: PokéAPI v2 pokemon/145
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
