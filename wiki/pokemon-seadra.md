---
title: "Seadra / 시드라"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-water", "evo-line-horsea"]
aliases: ["시드라", "seadra"]
summary: "#117 시드라 (물) — GO 최대 CP: 2,093"
evolution_line: "horsea"
---

# Seadra / 시드라

## Definition / 정의
**시드라** (#117)는 일반 포켓몬로, [[type-water|물]] 타입입니다.
"섣불리 만지려고 하면 몸 전체에서 돋아나는 가시에 찔려 기절할 수도 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #117 |
| 영문 이름 | Seadra |
| 타입 | [[type-water|물]] |
| 키 | 1.2m |
| 몸무게 | 25.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 75 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 55 |
| 공격 | 65 |
| 방어 | 95 |
| 특수공격 | 95 |
| 특수방어 | 45 |
| 스피드 | 85 |
| **합계** | **440** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-horsea|쏘드라]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-horsea]] |

## 특성 (Abilities)
- poison-point
- sniper
- damp

## Related Concepts / 관련 개념
- [[type-water|물 타입]] — 시드라의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 187 |
| 방어 (GO) | 156 |
| 체력 (GO) | 146 |
| 최대 CP (Lv.40) | 2,093 |
| 최대 CP (Lv.50) | 2,366 |
| 타입 | 물 |
| 버디 거리 | 3km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 풀, 전기 |
| 저항 | 강철, 불꽃, 물, 얼음 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-dragon-breath|용의숨결]], [[move-water-gun|물대포]]
- **스페셜 기술:** [[move-aurora-beam|오로라빔]], [[move-blizzard|눈보라]], [[move-dragon-pulse|용의파동]], [[move-hydro-pump|하이드로펌프]]  *(커뮤니티 데이/전용: [[move-blizzard|눈보라]])*

### 진화 체인
- **진화 전:** [[pokemon-horsea|쏘드라]]
- **진화 후:** [[pokemon-kingdra|킹드라]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| SEADRA_SHADOW | 187 | 156 | 146 | 2,093 |
## References
- Source: PokéAPI v2 pokemon/117
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
