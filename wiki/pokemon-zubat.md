---
title: "Zubat / 주뱃"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-poison", "type-flying", "evo-line-zubat"]
aliases: ["주뱃", "zubat"]
summary: "#041 주뱃 (독 / 비행) — GO 최대 CP: 667"
evolution_line: "zubat"
---

# Zubat / 주뱃

## Definition / 정의
**주뱃** (#041)는 일반 포켓몬로, [[type-poison|독]], [[type-flying|비행]] 타입입니다.
"입에서 내는 초음파로 두 눈이 없어도 주위의 장애물을 탐색할 수 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #041 |
| 영문 이름 | Zubat |
| 타입 | [[type-poison|독]], [[type-flying|비행]] |
| 키 | 0.8m |
| 몸무게 | 7.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 255 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 40 |
| 공격 | 45 |
| 방어 | 35 |
| 특수공격 | 30 |
| 특수방어 | 40 |
| 스피드 | 55 |
| **합계** | **245** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-golbat|골뱃]] |
| 계열 루트 | [[pokemon-zubat]] |

## 특성 (Abilities)
- inner-focus
- infiltrator

## Related Concepts / 관련 개념
- [[type-poison|독 타입]] — 주뱃의 타입
- [[type-flying|비행 타입]] — 주뱃의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 83 |
| 방어 (GO) | 73 |
| 체력 (GO) | 120 |
| 최대 CP (Lv.40) | 667 |
| 최대 CP (Lv.50) | 754 |
| 타입 | 독 / 비행 |
| 버디 거리 | 1km |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 바위, 전기, 에스퍼, 얼음 |
| 저항 | 격투(×¼), 독, 벌레(×¼), 풀(×¼), 페어리 |
| 무효 | 땅 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-bite|물기]], [[move-quick-attack|전광석화]]
- **스페셜 기술:** [[move-air-cutter|에어컷터]], [[move-poison-fang|독엄니]], [[move-sludge-bomb|오물폭탄]], [[move-swift|스피드스타]]  *(커뮤니티 데이/전용: [[move-sludge-bomb|오물폭탄]])*

### 진화 체인
- **진화 후:** [[pokemon-golbat|골뱃]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| ZUBAT_SHADOW | 83 | 73 | 120 | 667 |
## References
- Source: PokéAPI v2 pokemon/41
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
