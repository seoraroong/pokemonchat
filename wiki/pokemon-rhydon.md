---
title: "Rhydon / 코뿌리"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-ground", "type-rock", "evo-line-rhyhorn"]
aliases: ["코뿌리", "rhydon"]
summary: "#112 코뿌리 (땅/바위 타입) — 기본 스탯 합계 485"
evolution_line: "rhyhorn"
---

# Rhydon / 코뿌리

## Definition / 정의
**코뿌리** (#112)는 일반 포켓몬로, [[type-ground|땅]], [[type-rock|바위]] 타입입니다.
"전신을 갑옷 같은 피부로 보호하고 있다. 2000도의 마그마 속에서도 살 수 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #112 |
| 영문 이름 | Rhydon |
| 타입 | [[type-ground|땅]], [[type-rock|바위]] |
| 키 | 1.9m |
| 몸무게 | 120.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 60 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 105 |
| 공격 | 130 |
| 방어 | 120 |
| 특수공격 | 45 |
| 특수방어 | 45 |
| 스피드 | 40 |
| **합계** | **485** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-rhyhorn|뿔카노]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-rhyhorn]] |

## 특성 (Abilities)
- lightning-rod
- rock-head
- reckless

## Related Concepts / 관련 개념
- [[type-ground|땅 타입]] — 코뿌리의 타입
- [[type-rock|바위 타입]] — 코뿌리의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 222 |
| 방어 (GO) | 171 |
| 체력 (GO) | 233 |
| 최대 CP (Lv.40) | 3,179 |
| 최대 CP (Lv.50) | 3,594 |
| 타입 | 땅 / 바위 |
| 버디 거리 | 3km |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|345위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-mud-slap|진흙뿌리기]], [[move-rock-smash|바위깨기]]
- **스페셜 기술:** [[move-earthquake|지진]], [[move-megahorn|메가폰]], [[move-stone-edge|스톤에지]], [[move-surf|파도타기]], [[move-breaking-swipe|와이드브레이커]]  *(커뮤니티 데이/전용: [[move-megahorn|메가폰]])*

### 진화 체인
- **진화 전:** [[pokemon-rhyhorn|뿔카노]]
- **진화 후:** [[pokemon-rhyperior|거대코뿌리]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| RHYDON_SHADOW | 222 | 171 | 233 | 3,179 |
## References
- Source: PokéAPI v2 pokemon/112
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
