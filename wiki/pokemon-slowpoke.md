---
title: "Slowpoke / 야돈"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-water", "type-psychic", "evo-line-slowpoke"]
aliases: ["야돈", "slowpoke"]
summary: "#079 야돈 (물/에스퍼 타입) — 기본 스탯 합계 315"
evolution_line: "slowpoke"
---

# Slowpoke / 야돈

## Definition / 정의
**야돈** (#079)는 일반 포켓몬로, [[type-water|물]], [[type-psychic|에스퍼]] 타입입니다.
"항상 멍하니 있으므로 무슨 생각을 하고 있는지 알 수 없다. 꼬리로 먹이를 낚는 것이 특기다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #079 |
| 영문 이름 | Slowpoke |
| 타입 | [[type-water|물]], [[type-psychic|에스퍼]] |
| 키 | 1.2m |
| 몸무게 | 36.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 190 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 90 |
| 공격 | 65 |
| 방어 | 65 |
| 특수공격 | 40 |
| 특수방어 | 40 |
| 스피드 | 15 |
| **합계** | **315** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-slowbro|야도란]] |
| 계열 루트 | [[pokemon-slowpoke]] |

## 특성 (Abilities)
- oblivious
- own-tempo
- regenerator

## Related Concepts / 관련 개념
- [[type-water|물 타입]] — 야돈의 타입
- [[type-psychic|에스퍼 타입]] — 야돈의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 109 |
| 방어 (GO) | 98 |
| 체력 (GO) | 207 |
| 최대 CP (Lv.40) | 1,226 |
| 최대 CP (Lv.50) | 1,386 |
| 타입 | 물 / 에스퍼 |
| 버디 거리 | 3km |

### 기술 풀 (GO)
- **빠른 기술:** [[move-confusion|염동력]], [[move-water-gun|물대포]]
- **스페셜 기술:** [[move-psychic|사이코키네시스]], [[move-psyshock|사이코쇼크]], [[move-water-pulse|물의파동]]

### 진화 체인
- **진화 후:** [[pokemon-slowbro|야도란]], [[pokemon-slowking|야도킹]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| SLOWPOKE_GALARIAN | 109 | 98 | 207 | 1,226 |
| SLOWPOKE_SHADOW | 109 | 98 | 207 | 1,226 |
## References
- Source: PokéAPI v2 pokemon/79
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
