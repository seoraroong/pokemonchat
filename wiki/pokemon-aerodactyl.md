---
title: "Aerodactyl / 프테라"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-rock", "type-flying", "evo-line-aerodactyl"]
aliases: ["프테라", "aerodactyl"]
summary: "#142 프테라 (바위/비행 타입) — 기본 스탯 합계 515"
evolution_line: "aerodactyl"
---

# Aerodactyl / 프테라

## Definition / 정의
**프테라** (#142)는 일반 포켓몬로, [[type-rock|바위]], [[type-flying|비행]] 타입입니다.
"호박에 남아 있던 공룡의 유전자에서 부활시켰다. 높은 소리로 울며 난다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #142 |
| 영문 이름 | Aerodactyl |
| 타입 | [[type-rock|바위]], [[type-flying|비행]] |
| 키 | 1.8m |
| 몸무게 | 59.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 80 |
| 공격 | 105 |
| 방어 | 65 |
| 특수공격 | 60 |
| 특수방어 | 75 |
| 스피드 | 130 |
| **합계** | **515** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-aerodactyl]] |

## 특성 (Abilities)
- rock-head
- pressure
- unnerve

## Related Concepts / 관련 개념
- [[type-rock|바위 타입]] — 프테라의 타입
- [[type-flying|비행 타입]] — 프테라의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 221 |
| 방어 (GO) | 159 |
| 체력 (GO) | 190 |
| 최대 CP (Lv.40) | 2,783 |
| 최대 CP (Lv.50) | 3,147 |
| 타입 | 바위 / 비행 |
| 버디 거리 | 5km |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|438위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|288위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-bite|물기]], [[move-steel-wing|강철날개]], [[move-rock-throw|돌떨구기]], [[move-dragon-breath|용의숨결]]
- **스페셜 기술:** [[move-ancient-power|원시의힘]], [[move-hyper-beam|파괴광선]], [[move-iron-head|아이언헤드]], [[move-rock-slide|스톤샤워]], [[move-earth-power|대지의힘]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| AERODACTYL_MEGA | 292 | 210 | 190 | 4,118 |
| AERODACTYL_SHADOW | 221 | 159 | 190 | 2,783 |
## References
- Source: PokéAPI v2 pokemon/142
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
