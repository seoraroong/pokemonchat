---
title: "Electrode / 붐볼"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-electric", "evo-line-voltorb"]
aliases: ["붐볼", "electrode"]
summary: "#101 붐볼 (전기) — GO 최대 CP: 2,099"
evolution_line: "voltorb"
---

# Electrode / 붐볼

## Definition / 정의
**붐볼** (#101)는 일반 포켓몬로, [[type-electric|전기]] 타입입니다.
"작은 자극에도 반응해서 폭발한다. 폭탄볼이라 불리며 두려움의 대상이 되고 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #101 |
| 영문 이름 | Electrode |
| 타입 | [[type-electric|전기]] |
| 키 | 1.2m |
| 몸무게 | 66.6kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 60 |


## 도감 설명

> 작은 자극에도 반응해서 폭발한다. 폭탄볼이라 불리며 두려움의 대상이 되고 있다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 60 |
| 공격 | 50 |
| 방어 | 70 |
| 특수공격 | 80 |
| 특수방어 | 80 |
| 스피드 | 150 |
| **합계** | **490** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-voltorb|찌리리공]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-voltorb]] |

## 특성 (Abilities)
- soundproof
- static
- aftermath

## Related Concepts / 관련 개념
- [[type-electric|전기 타입]] — 붐볼의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 173 |
| 방어 (GO) | 173 |
| 체력 (GO) | 155 |
| 최대 CP (Lv.40) | 2,099 |
| 최대 CP (Lv.50) | 2,373 |
| 타입 | 전기 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|313위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|418위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅 |
| 저항 | 비행, 강철, 전기 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-spark|스파크]], [[move-tackle|몸통박치기]], [[move-volt-switch|볼트체인지]]
- **스페셜 기술:** [[move-discharge|방전]], [[move-hyper-beam|파괴광선]], [[move-thunderbolt|10만볼트]], [[move-foul-play|속임수]]  *(커뮤니티 데이/전용: [[move-tackle|몸통박치기]])*

### 진화 체인
- **진화 전:** [[pokemon-voltorb|찌리리공]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| ELECTRODE_HISUIAN | 176 | 176 | 155 | 2,149 |
| ELECTRODE_SHADOW | 173 | 173 | 155 | 2,099 |
## References
- Source: PokéAPI v2 pokemon/101
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
