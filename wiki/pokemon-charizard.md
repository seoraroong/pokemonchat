---
title: "Charizard / 리자몽"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-fire", "type-flying", "evo-line-charmander"]
aliases: ["리자몽", "charizard"]
summary: "#006 리자몽 (불꽃 / 비행) — GO 최대 CP: 2,889"
evolution_line: "charmander"
---

# Charizard / 리자몽

## Definition / 정의
**리자몽** (#006)는 일반 포켓몬로, [[type-fire|불꽃]], [[type-flying|비행]] 타입입니다.
"입에서 작렬하는 불꽃을 토해낼 때 꼬리의 끝이 더욱 붉고 격렬하게 타오른다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #006 |
| 영문 이름 | Charizard |
| 타입 | [[type-fire|불꽃]], [[type-flying|비행]] |
| 키 | 1.7m |
| 몸무게 | 90.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 78 |
| 공격 | 84 |
| 방어 | 78 |
| 특수공격 | 109 |
| 특수방어 | 85 |
| 스피드 | 100 |
| **합계** | **534** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-charmeleon|리자드]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-charmander]] |

## 특성 (Abilities)
- blaze
- solar-power

## Related Concepts / 관련 개념
- [[type-fire|불꽃 타입]] — 리자몽의 타입
- [[type-flying|비행 타입]] — 리자몽의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 223 |
| 방어 (GO) | 173 |
| 체력 (GO) | 186 |
| 최대 CP (Lv.40) | 2,889 |
| 최대 CP (Lv.50) | 3,266 |
| 타입 | 불꽃 / 비행 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|398위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|215위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|187위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 바위(×4), 물, 전기 |
| 저항 | 격투, 벌레(×¼), 강철, 불꽃, 풀(×¼), 페어리 |
| 무효 | 땅 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-air-slash|에어슬래시]], [[move-ember|불꽃세례]], [[move-fire-spin|회오리불꽃]], [[move-wing-attack|날개치기]], [[move-dragon-breath|용의숨결]]
- **스페셜 기술:** [[move-blast-burn|블러스트번]], [[move-dragon-claw|드래곤크루]], [[move-fire-blast|불대문자]], [[move-flamethrower|화염방사]], [[move-overheat|오버히트]], [[move-air-cutter|에어컷터]]  *(커뮤니티 데이/전용: [[move-ember|불꽃세례]], [[move-wing-attack|날개치기]], [[move-blast-burn|블러스트번]], [[move-flamethrower|화염방사]], [[move-dragon-breath|용의숨결]])*

### 진화 체인
- **진화 전:** [[pokemon-charmeleon|리자드]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| CHARIZARD_MEGA_X | 273 | 213 | 186 | 3,850 |
| CHARIZARD_MEGA_Y | 319 | 212 | 186 | 4,455 |
| CHARIZARD_SHADOW | 223 | 173 | 186 | 2,889 |
## References
- Source: PokéAPI v2 pokemon/6
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
