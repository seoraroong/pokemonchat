---
title: "Venusaur / 이상해꽃"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-grass", "type-poison", "evo-line-bulbasaur"]
aliases: ["이상해꽃", "venusaur"]
summary: "#003 이상해꽃 (풀 / 독) — GO 최대 CP: 2,720"
evolution_line: "bulbasaur"
---

# Venusaur / 이상해꽃

## Definition / 정의
**이상해꽃** (#003)는 일반 포켓몬로, [[type-grass|풀]], [[type-poison|독]] 타입입니다.
"큰 꽃잎을 펼쳐 햇빛을 받고 있으면 몸에 힘이 넘쳐흐른다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #003 |
| 영문 이름 | Venusaur |
| 타입 | [[type-grass|풀]], [[type-poison|독]] |
| 키 | 2.0m |
| 몸무게 | 100.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 80 |
| 공격 | 82 |
| 방어 | 83 |
| 특수공격 | 100 |
| 특수방어 | 100 |
| 스피드 | 80 |
| **합계** | **525** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-ivysaur|이상해풀]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-bulbasaur]] |

## 특성 (Abilities)
- overgrow
- chlorophyll

## Related Concepts / 관련 개념
- [[type-grass|풀 타입]] — 이상해꽃의 타입
- [[type-poison|독 타입]] — 이상해꽃의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 198 |
| 방어 (GO) | 189 |
| 체력 (GO) | 190 |
| 최대 CP (Lv.40) | 2,720 |
| 최대 CP (Lv.50) | 3,075 |
| 타입 | 풀 / 독 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|283위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|223위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|276위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 비행, 불꽃, 에스퍼, 얼음 |
| 저항 | 격투, 물, 풀(×¼), 전기, 페어리 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-razor-leaf|잎날가르기]], [[move-vine-whip|덩굴채찍]]
- **스페셜 기술:** [[move-frenzy-plant|하드플랜트]], [[move-petal-blizzard|꽃보라]], [[move-sludge-bomb|오물폭탄]], [[move-solar-beam|솔라빔]], [[move-sludge|오물공격]]  *(커뮤니티 데이/전용: [[move-frenzy-plant|하드플랜트]])*

### 진화 체인
- **진화 전:** [[pokemon-ivysaur|이상해풀]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| VENUSAUR_MEGA | 241 | 246 | 190 | 3,698 |
| VENUSAUR_SHADOW | 198 | 189 | 190 | 2,720 |
## References
- Source: PokéAPI v2 pokemon/3
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
