---
title: "Drowzee / 슬리프"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-psychic", "evo-line-drowzee"]
aliases: ["슬리프", "drowzee"]
summary: "#096 슬리프 (에스퍼 타입) — 기본 스탯 합계 328"
evolution_line: "drowzee"
---

# Drowzee / 슬리프

## Definition / 정의
**슬리프** (#096)는 일반 포켓몬로, [[type-psychic|에스퍼]] 타입입니다.
"잠들게 한 뒤 꿈을 먹지만 나쁜 꿈만 먹고 있으면 배탈이 날 때도 있는 것 같다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #096 |
| 영문 이름 | Drowzee |
| 타입 | [[type-psychic|에스퍼]] |
| 키 | 1.0m |
| 몸무게 | 32.4kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 190 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 60 |
| 공격 | 48 |
| 방어 | 45 |
| 특수공격 | 43 |
| 특수방어 | 90 |
| 스피드 | 42 |
| **합계** | **328** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-hypno|슬리퍼]] |
| 계열 루트 | [[pokemon-drowzee]] |

## 특성 (Abilities)
- insomnia
- forewarn
- inner-focus

## Related Concepts / 관련 개념
- [[type-psychic|에스퍼 타입]] — 슬리프의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 89 |
| 방어 (GO) | 136 |
| 체력 (GO) | 155 |
| 최대 CP (Lv.40) | 1,040 |
| 최대 CP (Lv.50) | 1,176 |
| 타입 | 에스퍼 |
| 버디 거리 | 3km |

### 기술 풀 (GO)
- **빠른 기술:** [[move-confusion|염동력]], [[move-pound|막치기]]
- **스페셜 기술:** [[move-psybeam|환상빔]], [[move-psychic|사이코키네시스]], [[move-psyshock|사이코쇼크]]

### 진화 체인
- **진화 후:** [[pokemon-hypno|슬리퍼]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| DROWZEE_SHADOW | 89 | 136 | 155 | 1,040 |
## References
- Source: PokéAPI v2 pokemon/96
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
