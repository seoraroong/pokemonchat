---
title: "Golbat / 골뱃"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-poison", "type-flying", "evo-line-zubat"]
aliases: ["골뱃", "golbat"]
summary: "#042 골뱃 (독/비행 타입) — 기본 스탯 합계 455"
evolution_line: "zubat"
---

# Golbat / 골뱃

## Definition / 정의
**골뱃** (#042)는 일반 포켓몬로, [[type-poison|독]], [[type-flying|비행]] 타입입니다.
"물리면 끝이다. 죽을 정도로 피를 빨아들이기 때문에 무거워져서 날 수 없게 될 때도 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #042 |
| 영문 이름 | Golbat |
| 타입 | [[type-poison|독]], [[type-flying|비행]] |
| 키 | 1.6m |
| 몸무게 | 55.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 90 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 75 |
| 공격 | 80 |
| 방어 | 70 |
| 특수공격 | 65 |
| 특수방어 | 75 |
| 스피드 | 90 |
| **합계** | **455** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-zubat|주뱃]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-zubat]] |

## 특성 (Abilities)
- inner-focus
- infiltrator

## Related Concepts / 관련 개념
- [[type-poison|독 타입]] — 골뱃의 타입
- [[type-flying|비행 타입]] — 골뱃의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 161 |
| 방어 (GO) | 150 |
| 체력 (GO) | 181 |
| 최대 CP (Lv.40) | 1,976 |
| 최대 CP (Lv.50) | 2,234 |
| 타입 | 독 / 비행 |
| 버디 거리 | 1km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|281위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-bite|물기]], [[move-wing-attack|날개치기]]
- **스페셜 기술:** [[move-air-cutter|에어컷터]], [[move-ominous-wind|괴상한바람]], [[move-poison-fang|독엄니]], [[move-shadow-ball|섀도볼]]  *(커뮤니티 데이/전용: [[move-ominous-wind|괴상한바람]])*

### 진화 체인
- **진화 전:** [[pokemon-zubat|주뱃]]
- **진화 후:** [[pokemon-crobat|크로뱃]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| GOLBAT_SHADOW | 161 | 150 | 181 | 1,976 |
## References
- Source: PokéAPI v2 pokemon/42
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
