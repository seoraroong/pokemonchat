---
title: "Magneton / 레어코일"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-electric", "type-steel", "evo-line-magnemite"]
aliases: ["레어코일", "magneton"]
summary: "#082 레어코일 (전기 / 강철) — GO 최대 CP: 2,485"
evolution_line: "magnemite"
---

# Magneton / 레어코일

## Definition / 정의
**레어코일** (#082)는 일반 포켓몬로, [[type-electric|전기]], [[type-steel|강철]] 타입입니다.
"다수의 코일이 연결되어 강력한 자기장과 높은 전압을 방사한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #082 |
| 영문 이름 | Magneton |
| 타입 | [[type-electric|전기]], [[type-steel|강철]] |
| 키 | 1.0m |
| 몸무게 | 60.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 60 |


## 도감 설명

> 다수의 코일이 연결되어 강력한 자기장과 높은 전압을 방사한다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 50 |
| 공격 | 60 |
| 방어 | 95 |
| 특수공격 | 120 |
| 특수방어 | 70 |
| 스피드 | 70 |
| **합계** | **465** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-magnemite|코일]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-magnemite]] |

## 특성 (Abilities)
- magnet-pull
- sturdy
- analytic

## Related Concepts / 관련 개념
- [[type-electric|전기 타입]] — 레어코일의 타입
- [[type-steel|강철 타입]] — 레어코일의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 223 |
| 방어 (GO) | 169 |
| 체력 (GO) | 137 |
| 최대 CP (Lv.40) | 2,485 |
| 최대 CP (Lv.50) | 2,810 |
| 타입 | 전기 / 강철 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|414위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|323위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 격투, 땅(×4), 불꽃 |
| 저항 | 노말, 비행(×¼), 바위, 벌레, 강철(×¼), 풀, 전기, 에스퍼, 얼음, 드래곤, 페어리 |
| 무효 | 독 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-charge-beam|차지빔]], [[move-spark|스파크]], [[move-thunder-shock|전기쇼크]], [[move-volt-switch|볼트체인지]], [[move-metal-sound|금속음]]
- **스페셜 기술:** [[move-discharge|방전]], [[move-flash-cannon|러스터캐논]], [[move-magnet-bomb|마그넷봄]], [[move-zap-cannon|전자포]]

### 진화 체인
- **진화 전:** [[pokemon-magnemite|코일]]
- **진화 후:** [[pokemon-magnezone|자포코일]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| MAGNETON_SHADOW | 223 | 169 | 137 | 2,485 |
## References
- Source: PokéAPI v2 pokemon/82
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
