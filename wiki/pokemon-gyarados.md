---
title: "Gyarados / 갸라도스"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-water", "type-flying", "evo-line-magikarp"]
aliases: ["갸라도스", "gyarados"]
summary: "#130 갸라도스 (물/비행 타입) — 기본 스탯 합계 540"
evolution_line: "magikarp"
---

# Gyarados / 갸라도스

## Definition / 정의
**갸라도스** (#130)는 일반 포켓몬로, [[type-water|물]], [[type-flying|비행]] 타입입니다.
"분쟁이 일어난 마을을 다 태워버렸다는 기록이 고문서에 남아 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #130 |
| 영문 이름 | Gyarados |
| 타입 | [[type-water|물]], [[type-flying|비행]] |
| 키 | 6.5m |
| 몸무게 | 235.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 95 |
| 공격 | 125 |
| 방어 | 79 |
| 특수공격 | 60 |
| 특수방어 | 100 |
| 스피드 | 81 |
| **합계** | **540** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-magikarp|잉어킹]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-magikarp]] |

## 특성 (Abilities)
- intimidate
- moxie

## Related Concepts / 관련 개념
- [[type-water|물 타입]] — 갸라도스의 타입
- [[type-flying|비행 타입]] — 갸라도스의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 237 |
| 방어 (GO) | 186 |
| 체력 (GO) | 216 |
| 최대 CP (Lv.40) | 3,391 |
| 최대 CP (Lv.50) | 3,834 |
| 타입 | 물 / 비행 |
| 버디 거리 | 1km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|145위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|51위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|57위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-bite|물기]], [[move-dragon-breath|용의숨결]], [[move-dragon-tail|드래곤테일]], [[move-waterfall|폭포오르기]]
- **스페셜 기술:** [[move-crunch|깨물어부수기]], [[move-dragon-pulse|용의파동]], [[move-hydro-pump|하이드로펌프]], [[move-outrage|역린]], [[move-twister|회오리]], [[move-aqua-tail|아쿠아테일]]  *(커뮤니티 데이/전용: [[move-dragon-tail|드래곤테일]], [[move-dragon-pulse|용의파동]], [[move-aqua-tail|아쿠아테일]])*

### 진화 체인
- **진화 전:** [[pokemon-magikarp|잉어킹]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| GYARADOS_MEGA | 292 | 247 | 216 | 4,717 |
| GYARADOS_SHADOW | 237 | 186 | 216 | 3,391 |
## References
- Source: PokéAPI v2 pokemon/130
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
