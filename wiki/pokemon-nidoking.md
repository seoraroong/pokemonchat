---
title: "Nidoking / 니드킹"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-poison", "type-ground", "evo-line-nidoran-m"]
aliases: ["니드킹", "nidoking"]
summary: "#034 니드킹 (독 / 땅) — GO 최대 CP: 2,567"
evolution_line: "nidoran-m"
---

# Nidoking / 니드킹

## Definition / 정의
**니드킹** (#034)는 일반 포켓몬로, [[type-poison|독]], [[type-ground|땅]] 타입입니다.
"돌처럼 딱딱한 피부와 길게 뻗은 뿔이 특징이다. 뿔에는 독이 있으니 주의해야 한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #034 |
| 영문 이름 | Nidoking |
| 타입 | [[type-poison|독]], [[type-ground|땅]] |
| 키 | 1.4m |
| 몸무게 | 62.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |


## 도감 설명

> 돌처럼 딱딱한 피부와 길게 뻗은 뿔이 특징이다. 뿔에는 독이 있으니 주의해야 한다.
## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 81 |
| 공격 | 102 |
| 방어 | 77 |
| 특수공격 | 85 |
| 특수방어 | 75 |
| 스피드 | 85 |
| **합계** | **505** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-nidorino|니드리노]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-nidoran-m]] |

## 특성 (Abilities)
- poison-point
- rivalry
- sheer-force

## Related Concepts / 관련 개념
- [[type-poison|독 타입]] — 니드킹의 타입
- [[type-ground|땅 타입]] — 니드킹의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 204 |
| 방어 (GO) | 156 |
| 체력 (GO) | 191 |
| 최대 CP (Lv.40) | 2,567 |
| 최대 CP (Lv.50) | 2,902 |
| 타입 | 독 / 땅 |
| 버디 거리 | 3km |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|470위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 땅, 물, 에스퍼, 얼음 |
| 저항 | 격투, 독(×¼), 바위, 벌레, 페어리 |
| 무효 | 전기 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-fury-cutter|연속자르기]], [[move-iron-tail|아이언테일]], [[move-poison-jab|독찌르기]], [[move-double-kick|두번치기]], [[move-poison-sting|독침]]
- **스페셜 기술:** [[move-earthquake|지진]], [[move-megahorn|메가폰]], [[move-sludge-wave|오물웨이브]], [[move-earth-power|대지의힘]], [[move-sand-tomb|모래지옥]]  *(커뮤니티 데이/전용: [[move-fury-cutter|연속자르기]])*

### 진화 체인
- **진화 전:** [[pokemon-nidorino|니드리노]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| NIDOKING_SHADOW | 204 | 156 | 191 | 2,567 |
## References
- Source: PokéAPI v2 pokemon/34
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
