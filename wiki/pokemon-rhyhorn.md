---
title: "Rhyhorn / 뿔카노"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-ground", "type-rock", "evo-line-rhyhorn"]
aliases: ["뿔카노", "rhyhorn"]
summary: "#111 뿔카노 (땅/바위 타입) — 기본 스탯 합계 345"
evolution_line: "rhyhorn"
---

# Rhyhorn / 뿔카노

## Definition / 정의
**뿔카노** (#111)는 일반 포켓몬로, [[type-ground|땅]], [[type-rock|바위]] 타입입니다.
"머리는 나쁘지만 힘이 세서 고층 빌딩도 몸통박치기로 산산조각 낸다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #111 |
| 영문 이름 | Rhyhorn |
| 타입 | [[type-ground|땅]], [[type-rock|바위]] |
| 키 | 1.0m |
| 몸무게 | 115.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 120 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 80 |
| 공격 | 85 |
| 방어 | 95 |
| 특수공격 | 30 |
| 특수방어 | 30 |
| 스피드 | 25 |
| **합계** | **345** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | 없음 |
| 진화 후 | [[pokemon-rhydon|코뿌리]] |
| 계열 루트 | [[pokemon-rhyhorn]] |

## 특성 (Abilities)
- lightning-rod
- rock-head
- reckless

## Related Concepts / 관련 개념
- [[type-ground|땅 타입]] — 뿔카노의 타입
- [[type-rock|바위 타입]] — 뿔카노의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 140 |
| 방어 (GO) | 127 |
| 체력 (GO) | 190 |
| 최대 CP (Lv.40) | 1,651 |
| 최대 CP (Lv.50) | 1,867 |
| 타입 | 땅 / 바위 |
| 버디 거리 | 3km |

### 기술 풀 (GO)
- **빠른 기술:** [[move-mud-slap|진흙뿌리기]], [[move-rock-smash|바위깨기]]
- **스페셜 기술:** [[move-bulldoze|땅고르기]], [[move-horn-attack|뿔찌르기]], [[move-stomp|짓밟기]]

### 진화 체인
- **진화 후:** [[pokemon-rhydon|코뿌리]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| RHYHORN_SHADOW | 140 | 127 | 190 | 1,651 |
## References
- Source: PokéAPI v2 pokemon/111
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
