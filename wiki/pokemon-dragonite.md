---
title: "Dragonite / 망나뇽"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-05
tags: ["pokemon", "gen1", "type-dragon", "type-flying", "evo-line-dratini"]
aliases: ["망나뇽", "dragonite"]
summary: "#149 망나뇽 (드래곤/비행 타입) — 기본 스탯 합계 600"
evolution_line: "dratini"
---

# Dragonite / 망나뇽

## Definition / 정의
**망나뇽** (#149)는 일반 포켓몬로, [[type-dragon|드래곤]], [[type-flying|비행]] 타입입니다.
"커다란 몸집으로 하늘을 난다. 지구를 약 16시간 만에 일주한다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #149 |
| 영문 이름 | Dragonite |
| 타입 | [[type-dragon|드래곤]], [[type-flying|비행]] |
| 키 | 2.2m |
| 몸무게 | 210.0kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 91 |
| 공격 | 134 |
| 방어 | 95 |
| 특수공격 | 100 |
| 특수방어 | 100 |
| 스피드 | 80 |
| **합계** | **600** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-dragonair|신뇽]] |
| 진화 후 | 없음 |
| 계열 루트 | [[pokemon-dratini]] |

## 특성 (Abilities)
- inner-focus
- multiscale

## Related Concepts / 관련 개념
- [[type-dragon|드래곤 타입]] — 망나뇽의 타입
- [[type-flying|비행 타입]] — 망나뇽의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 263 |
| 방어 (GO) | 198 |
| 체력 (GO) | 209 |
| 최대 CP (Lv.40) | 3,792 |
| 최대 CP (Lv.50) | 4,287 |
| 타입 | 드래곤 / 비행 |
| 버디 거리 | 5km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|325위]] |
| PvP 순위 (하이퍼리그 (UL 2500)) | [[pvp-ul\|299위]] |
| PvP 순위 (마스터리그 (ML)) | [[pvp-ml\|62위]] |

### 기술 풀 (GO)
- **빠른 기술:** [[move-dragon-breath|용의숨결]], [[move-dragon-tail|드래곤테일]], [[move-steel-wing|강철날개]]
- **스페셜 기술:** [[move-draco-meteor|용성군]], [[move-dragon-claw|드래곤크루]], [[move-dragon-pulse|용의파동]], [[move-hurricane|폭풍]], [[move-hyper-beam|파괴광선]], [[move-outrage|역린]], Super Power, [[move-thunder-punch|번개펀치]]  *(커뮤니티 데이/전용: [[move-draco-meteor|용성군]], [[move-dragon-pulse|용의파동]])*

### 진화 체인
- **진화 전:** [[pokemon-dragonair|신뇽]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| DRAGONITE_MEGA | 299 | 255 | 209 | 4,823 |
| DRAGONITE_SHADOW | 263 | 198 | 209 | 3,792 |
## References
- Source: PokéAPI v2 pokemon/149
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
