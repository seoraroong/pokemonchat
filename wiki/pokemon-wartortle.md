---
title: "Wartortle / 어니부기"
type: concept
language: ko
created: 2026-06-05
modified: 2026-06-18
tags: ["pokemon", "gen1", "type-water", "evo-line-squirtle"]
aliases: ["어니부기", "wartortle"]
summary: "#008 어니부기 (물) — GO 최대 CP: 1,488"
evolution_line: "squirtle"
---

# Wartortle / 어니부기

## Definition / 정의
**어니부기** (#008)는 일반 포켓몬로, [[type-water|물]] 타입입니다.
"딱 하고 머리를 맞을 때 등껍질로 숨어서 피한다. 하지만 꼬리가 살짝 삐져나와 있다."

## 기본 정보

| 항목 | 값 |
|------|-----|
| 도감 번호 | #008 |
| 영문 이름 | Wartortle |
| 타입 | [[type-water|물]] |
| 키 | 1.0m |
| 몸무게 | 22.5kg |
| 분류 | 일반 포켓몬 |
| 포획률 | 45 |

## 기본 스탯 (Base Stats)

| 스탯 | 수치 |
|------|------|
| HP | 59 |
| 공격 | 63 |
| 방어 | 80 |
| 특수공격 | 65 |
| 특수방어 | 80 |
| 스피드 | 58 |
| **합계** | **405** |

## 진화 정보

| 항목 | 포켓몬 |
|------|--------|
| 진화 전 | [[pokemon-squirtle|꼬부기]] |
| 진화 후 | [[pokemon-blastoise|거북왕]] |
| 계열 루트 | [[pokemon-squirtle]] |

## 특성 (Abilities)
- torrent
- rain-dish

## Related Concepts / 관련 개념
- [[type-water|물 타입]] — 어니부기의 타입
- [[pokedex-gen1]] — 1세대 포켓몬 목록
- [[type-chart]] — 타입 상성표

## 포켓몬 GO 스탯

| 항목 | 수치 |
|------|------|
| 공격 (GO) | 126 |
| 방어 (GO) | 155 |
| 체력 (GO) | 153 |
| 최대 CP (Lv.40) | 1,488 |
| 최대 CP (Lv.50) | 1,682 |
| 타입 | 물 |
| 버디 거리 | 3km |
| PvP 순위 (슈퍼리그 (GL 1500)) | [[pvp-gl\|275위]] |


### 타입 상성 (GO 기준)

| 구분 | 타입 |
|------|------|
| 약점 | 풀, 전기 |
| 저항 | 강철, 불꽃, 물, 얼음 |
### 기술 풀 (GO)
- **빠른 기술:** [[move-bite|물기]], [[move-water-gun|물대포]]
- **스페셜 기술:** [[move-aqua-jet|아쿠아제트]], [[move-hydro-pump|하이드로펌프]], [[move-ice-beam|냉동빔]]

### 진화 체인
- **진화 전:** [[pokemon-squirtle|꼬부기]]
- **진화 후:** [[pokemon-blastoise|거북왕]]

### 변형 폼
| 폼 | 공격 | 방어 | 체력 | 최대 CP(40) |
|-----|------|------|------|------------|
| WARTORTLE_SHADOW | 126 | 155 | 153 | 1,488 |
## References
- Source: PokéAPI v2 pokemon/8
- Hash: `bb8096ad5a0b2c74200879a8a90f450de39cf76898c0fd158d85011302b5f3d5`
